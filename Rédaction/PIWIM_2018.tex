...
We build on three sectoral world input-output datasets (WIOD and two versions of TiVA) using Cobb-Douglas production functions in a partial equilibrium setting.
...
Depending on countries, the absolute value of the elasticity of the CPI to the exchange rate ranges from 5\% to 35\%, confirming the importance of global value chains in channeling external shocks to domestic inflation.
Most of the propagation of shocks passes throught imported final goods and domestic input-output linkages.
Using data from WIOD on a sample of 43 countries, we find that the mean output-weighed elasticity of the CPI to the exchange rate increased in absolute value from 7.5\% in 2000 to 9.4\% in 2008. 
After peaking in 2008, it declined to 8.8\% in 2014.
...
%HC : AJout 08/2020
%VF 27 Sept il aurait fallu des réferences pour introduire le débat: Ball et Mazumder (2011), Coibion and Gorodnichenko (2013), voire Mojon pour global inflation... Je propose de situer les 2 débats en ajouter les repères temporels a minima
...
The “missing inflation puzzle" emerged from 2012 onwards, as inflation remained subdued despite the economic recovery.  
...
Assuming Cobb-Douglas production functions where firms have a simple cost-minimising behaviour, we compute the partial-equilibrium effects of an exchange rate shock on consumer price indexes.
...
Leontief's production model is dual, but we will focus on the first one: the cost-push price model. Using it, some studies focus on the consequences of a change in production prices based on an I-O model or a SAM (Social Accounting Matrix) model in developing countries.
%GD20200620 IL FAUT ALLER PLUS LOIN ET CITER DES SOURCES, NON ?
...
The WIOD and TiVA databases have distinct characteristics (see \cite{Timmer2015}). WIOD assumes that the input mix is independant of the destination market. 
%VF 27 Sept je ne comprend pas ce qui dépend de quoi dans la dernière partie de la phrase ci-dessous
TiVA does not rely on this assumption for two countries (Mexico and China), using for both countries different input-output table for the exporting sector and the import processing sector, the domestic-market oriented sector depending on the country and the version of the TiVA database.\\
...
\noindent\fbox{%
	\parbox{\textwidth}{%
		...
		\item ${\cal A}$: Matrix of world input-output coefficients.
				...
				\item $c^i$: exchange rate shock on the price of non-$i$ goods expressed in the currency of $i$. $c^i= -\frac{c_\$^i}{1+c_\$^i}$. If $c_\$^i = 0.1$, $c^i=-0.05$ 
...
We assume taht the shock is small enough to assimilate log and percentage points and hence consider $\overline{s_{i}}^{i,HC}$ as the elasticity of the CPI to the excahnge rate. Following an appreciation of the national currency versus all other currencies, imported inputs and imported consumer goods become cheaper and domestic prices expressed in national currency decrease. 
...
Using WIOD, we find that the absolute value of the elasticity of the consumer price index to exchange rate shocks in 0.055 (or 5.5\%) for the US in 2011
...
Output-weighed results show a lower elasticity, reflecting the fact that large countries are relatively closed compared to small open economies.
Based on the third revision of TiVA database, we find that the output-weighed elasticity has increased by 25\% between 1995 and 2008, to 10\% in 2008. 
...
\begin{figure}[!h]
	\centering
	\caption{\footnotesize{\textbf{Elasticity of consumer prices to an exchange rate shock and the share of imported consumption in total consumption (WIOD)}}}
	\begin{tabular}{c}
...
On the whole, as shown by Figure \ref{fig:shareofs}, input-output mechasims (i.e. everything but $E1.HC$) explain a large share of the elasticity, especially for large countries or countries of the Eurozone subject to a shock on the Euro. This share has increased untill 2013-2014, implying an increasing need of data from WIOTs to perform our computations (see Figure \ref{fig:shareofsthroughtime}).
...
A less demanding test is to run a panel with country fixed-effects, assuming that $\beta$ is constant over time but that it explains only within-country variations. To take into account year-specific shocks, we add two year-specific variables : the GDP-weighed mean of each variable of interest. (see equation \ref{eq:eq9}). 
...
Hence, we could use this approach to estimate the elasticity of consumer prices to the exchnge rate from 2015 onwards.
...
Using these equations, we can predict the elasticity of consumer prices for 2016 onwards to make up for the lack of WIOTs.
Figure \ref{fig:panel_pred3} shows the predictions. The in-sample predictions seem rather rather robust, giving us confidence in the quality of the out-of-sample predictions.


\begin{figure}[!h]
	\centering
	\caption{\footnotesize{\textbf{Comparing the output-weighed CPI elasticity to its prediction without using Eurostat data.}}}
	\begin{tabular}{c}
...
The elasticity of domestic consumer price index to a shock on the domestic currency ranges from 5\% and 35\%, depending on the openness of countries.
...
First-round effects explain three-quarters of the propagation of global shocks to domestic prices.
...
We analyse the impact of global inflationary shocks on the main components of the CPI (manufacturing goods, services, food and energy) and find that non-energy industrial goods make the bulk of the total impact. Services also play a significant role, especially in advanced economies. Indeed, although services are mainly produced domestically and do not rely much on imported inputs, they make up a substantial share of total consumption.
As a result, domestic core inflation (defined as inflation excluding food and energy) accounts for a significant share of the total impact, reflecting the weight of domestic services and non-energy industrial goods in total consumption.
...
We thus provide a tool for approximating the elasticity of the CPI to an exchange rate shock from 2015 onwards.
