...
\cite{Cochard2016} relies on an accounting approach to analyse the effect of costs on prices ("cost-push inflation"). 
...
Two datasets are available: the World Input Output Database (WIOD) and the OECD-ICIO database TiVA.
...
To formalise the initial impact of the shock on the price of traded goods, we neutralise the impact of an input price shock on the price of domestic inputs as well as on the price of inputs traded between countries that are not shocked.\\
...
However, using data from TIVA rev. 3 yields a slightly higher elasticity (see Figure \ref{fig:PIWIM_LONGITUDINAL}).
...
While the World Bank provides regular estimates for household consumption, it does not provide an estimate for intermediate consumptions. In the case of European countries\footnote{And some others for selected years}, Eurostat provides estimates for intermediate consumptions.%\footnote{préciser la source}.
...
Figure \ref{fig:tous_les_E} comfirms that, in that numerical exercise, the total effect is dominated by the direct effect through imported consumption goods and, to a lesser extent, the effect on domestic consumption goods through imported inputs. 
