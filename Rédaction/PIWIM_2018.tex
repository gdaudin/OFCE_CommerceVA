...
By contrast, we find a limited role for the second-round effect, i.e. the additional transmission of price changes of the domestic inputs to other sectors of the domestic economy and other countries occurring during subsequent production cycles. 
...
To address this gap, we use up-to-date GDP and trade data to approximate the accounting impact of an exchange rate shock on consumer price index for the most recent years.
...
In figure 2, we built an average CPI elasticity of the whole country sample. The anual evolution is there too quite regardless of the database (WIOD or TIVA) 
% Figure 1 à vérifier ie erreur dans les légendes des axes. Proposition de titre du figure 1: comparison of the CPI elasticity to an exchange rate shock for WIOD, TIVA and TIVA rev.4 for 2011 and 2014
% Figure 2: comparison of the average CPI elasticity* of the country sample to an exchange rate shock for WIOD and TIVA. * The average CPI elasticity has been computed from the 43 countries sample(no Eurozone) and aggregated from a arrhythmic mean and output weighted
\begin{figure}[!h]
\centering
...
% Figure 3: Distribution of the CPI elasticity to a 100 % exchange rate appreciation (WIOD) - 2014. Note: each country is supposed to have its local currency (maybe hypothetical), except for _EUR. The shock is theon the Euro. 
\begin{figure}[!h]
	\centering
...
%  faute d'orthograpphe dans la légende des abscisses du titre 9: effect et pas effet
...
%Figure 13: Comparison the CPI elasticity in 2014 (WIOD) and the prediction from a panel regression on the 2000-2008 period with fixed effects. Note: beta is constant over time for each country
 \begin{equation}
\begin{array}{llll}
