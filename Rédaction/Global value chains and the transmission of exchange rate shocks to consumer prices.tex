...
As WIOT are data-demanding and available with a lag of several years, we extrapolate a reliable year- and country-specific estimate of the HCE deflator elasticity from 2015 onwards, using trade data and GDP statistics. 
...
%Data and method
We perform an accounting exercise based on information contained in world input-output tables (WIOTs hereafter), by way of large matrices inversions.
...
Our paper contributes to both strands of literature. 
...
Nevertheless, our results broadly concur with the recent literature.
...
The estimated elasticity of the CPI to the exchange rate ranges from 0.07 for the US to 0.34 for Ireland, with a simple average of 0.18 on a sample of 26 countries.\\
...
For example, we estimate the accounting impact of an appreciation of the US dollar.\\
% Impact par secteur
...
Although the MRIO dataset covers most recent years, it suffers from data quality issues from 2018. 
...
Section \ref{sec:data}, we review the data sources and outlines the main differences between the various WIOTs.
Section \ref{sec:metho} we present our method.
...
\cite{Colavecchio2020} focus on the ERPT to consumer prices in the euro area from 1997 to 2019 using local projection techniques.\footnote{To the best of our knowledge, the literature using WIOTs mostly focuses on the transmission of production shocks through input-output linkages. By contrast, we use WIOTs to analyse cost-push inflation. As a result, and despite differences in methodologies, we chiefly rely on the ERPT literature to compare our estimates of the HCE deflator elasticity with the existing literature.} 
...
\paragraph{Caveats}
As underlined in Section \ref{subsec:prixconso}, our computations rely on several simplifying assumptions which bias our estimates upwards. 
