...
graph dot (asis) pond_WIOD_HC if sample==1,  over(elast_order, ///
	label(labsize(tiny))) marker(1, ms(O) mfcolor(gs1) mlcolor(red) msize(tiny)) ///
	legend(off) title("WIOD elasticites 2014 (absolute value)") ///
	...
	scheme(scolor) xsize(6)  ysize(7)

graph export "$dirgit/Rédaction/Vox_EU_graph1.png", replace
drop sample
