...
We study the impact of exchange rate shocks on consumer price index by using three sectoral world input-output datasets.
We assume a Leontieff framework with no substitution between different consumer goods or different inputs.
Firms react to a price increase of inputs by adjusting their prices to maintain their value-added. 
In a sample of 43 countries, the mean output-weighted elasticity of the CPI to exchange rate shocks according to WIOD increased in absulte value from 0.075 in 2000 to 0.094 in 2008. It then declined to 0.088 in 2014.
Both recent versions of TIVA yield a similar evolution.
A defect of our approach is that world input-output databases are only available after a sizeable time gap (currently five years). 
However, we show that it is possible to use up-to-date GDP and trade data to extrapolate the accounting impact of exchange rate shock on consumer price index.
...
Our results suggest that we can approximate the elasticity of consumer prices for the most recent years, using the share of imported goods in household consumption and the share of imported inputs in household consumption of domestic goods. The first-round effects are a good predictor of the total effects.
Yet, care should be taken in extrapolating them as most of the non-first round effects add to the first round rather than amplifying them. They are of similar size for small open economies and large closed ones.
