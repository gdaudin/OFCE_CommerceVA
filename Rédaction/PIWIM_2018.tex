...
Yet, these two data pieces are not actually easy to obtain for a large number of countries and a large number of years. They are not routinely computed by national statistical institutes. The World Bank provides regular estimates of household consumption.\footnote{préciser la source??}. It does not provide an estimate for intermediate consumptions. In the case of European countries\footnote{And some others for selected years}, Eurostat does provide this last number.

We can combine these data from the World Bank and Eurostat with UN Comtrade data for imports in consumption and intermediary goods (using the BEC classification). That allows the computation of the share of imported consumption goods in household consumption and the share of imported inputs in all inputs. 

This allows the mimicking of Figure \ref{fig:evolution_b} by Figure \ref{fig:evolution_b_reg2}. The results are not as good.	


\begin{figure}[!h]
	\centering
	\caption{\footnotesize{\textbf{Evolution of $\beta$ and $R2$ through time (WIOD) using Eurostat data}}}
	\begin{tabular}{c}
		\includegraphics[width=5.0in, height=3.5in]{reg1_beta_WIOD.png}\\
	\end{tabular}
