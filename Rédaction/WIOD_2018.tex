...
Define $Y$ the output vector of dimension $(1, N)$, $A$ the matrix of technical input coefficient of dimension $(N, N)$, and $R$ the vector of factor incomes of dimension $(1, N)$. $N$ is equal to the product of the number of countries ($I$) and the number of sectors ($J$). \\
...
%\textcolor{red} {GD CE QUI SUIT ME SEMBLE TRÈS BIZARRE -- Je refais un essai} \\
...
Define $P$ the vector of production prices of dimension $(1, N)$ and $V$ the vector of factor income of dimension $(1, N)$ needed to produce one unit of good in each sector. Then $P=PA+V$. \\

%\textcolor{red} {Ici} \\
 % Define $y_i=p_i*q_i$, with $p_i$ the price and $q_i$ the quantity of product $i$ and normalize quantities such as $p_i=1$. \\
%Define $P$ the vector of production prices of dimension $(1, n)$ and and $V$ the vector of factor income per quantity unit of production of dimension $(1, n)$. The coefficients of structural %matrix of technical coefficients of dimension $(n,n)$ are the same as the coefficients of the structural matrix of input coefficients because $p_i=1$.  We call it $A$ as well. Then $P=PA+V$. \\

%\textcolor{red} {Retour au texte} \\
...
In a second step, the shock is passed on all country-specific industries using these shocked inputs in their production processes. For $k$ production cycles, the increase in production prices amounts to $\Delta^k P=CA^k$.\\
...
The total effect of the shock on prices, S, is equal to $S=C {\left( I+A+A^2+...+A^k+...\right)} =C {\left( I-A \right)}^{-1}$, with ${\left( I-A \right)}^{-1}$ the inverse of Leontief's matrix; $S$ is a vector $(1, N)$ composed of the elements $s_{ij}$ measuring the total effect of the shock on the output price of sector $j$ in country $i$ .
...
Building on Equation \ref{eq:eq1}, we write the direct impact of the exchange rate shock on the other countries as the product of the shock vector $C_\$^i$ and a matrix $B$. $B$ builds on the large matrix $A$ of technical coefficients, but only keeps the coefficients of each country's sectoral inputs imported from the shocked country $i$. The other coefficients are replaced by 0, including those of the block of country $i$ concerning the domestic inputs of the shocked country $i$. The direct impact of the appreciation of a currency against the dollar on the price of inputs in countries that are not shocked is equal to $C_\$^iB$ with
\begin{eqnarray}
C_\$^iB=\left(0\ldots c_\$^i \ldots 0\right)\left(\begin{matrix}0&\cdots&0\\a_{11,ij}&0&a_{IJ,ij}\\0&\cdots&0\\\end{matrix}\right) 	
\label{eq:eq3}
\end{eqnarray}
...
Define $C^i$ the vector of change in input prices imported by country i, in national currency, 
$\left(-\frac{c_\$^i}{1+c_\$^i},\ldots0\ldots,-\frac{c_\$^i}{1+c_\$^i} \right)$. \\

From Equation \ref{eq:eq2}, we can write the direct impact for country $i$ of the fall in input prices from the rest of the world. The direct impact corresponds to the product of the shock vector $C^i$  and a matrix ${\tilde{B}}$. ${\tilde{B}}$ builds on the large matrix A of which only the country blocks of the inputs imported by country $i$ from other countries have been retained. The other coefficients are replaced by 0, including those of the block of country $i$ concerning the domestic inputs of the shocked country $i$. \\

The direct impact of the appreciation of the shocked country $i$ on the price of its inputs corresponds, in national currency, to $C^i{\tilde{B}}$ with: 
\begin{eqnarray}
C^i{\tilde{B}}=\left(-\frac{c_\$^i}{1+c_\$^i},\ldots0\ldots,-\frac{c_\$^i}{1+c_\$^i} \right)\left(\begin{matrix}0&\ldots{{{a}}_{ij,11}}\ldots&0\\0&0&0\\0&\ldots{{a}_{{ij,IJ}}}\ldots&0\\\end{matrix}\right) 
\label{eq:eq4}
 \end{eqnarray}
...
We can then convert this direct impact in dollars, by multiplying it by the new value of the national currency in dollars, $\left(1+c_\$^i\right)$. The direct impact of the appreciation of the shocked country $i$ on the price of its inputs corresponds, in dollars, to ${\tilde{C}_\$^i}{\tilde{B}}$ with: 
\begin{eqnarray}
{\tilde{C}_\$^i}{\tilde{B}}=\left(1+c_\$^i\right).C^i{\tilde{B}}=
...
The direct effect on the world is therefore the sum of these vectors from equations  \ref{eq:eq3} and \ref{eq:eq4}, i. e. $C_\$^i.B+{\tilde{C}_\$^i}{\tilde{B}}.~$\\
...
\begin{eqnarray*}
S_\$^i=\Delta{{P}_{\$}^i}=C_\$^i+\left(C_\$^i.B+{\tilde{C}_\$^i}{\tilde{B}}\right){A}+\left(C_\$^i.B+{\tilde{C}_\$^i}{\tilde{B}}\right){{{A}}^{2}}+\ldots+\left(C_\$^i.B+{\tilde{C}_\$^i}{\tilde{B}}\right)A^k +\ldots \\
\end{eqnarray*}
 
 \begin{eqnarray}
S_\$^i=C_\$^i+\left(C_\$^i.B+{\tilde{C}_\$^i}{\tilde{B}}\right)*{{(I-A)}^{-1}}	
\label{eq:eq5}
 \end{eqnarray}
...
Equation \ref{eq:eq5} gives the absolute evolution of input prices in international currency. To obtain the absolute evolution of the input prices of the shocked country in national currency, we remove the exchange rate shock and multiply this balance by the scalar of conversion equal to $\frac{1}{1+c_\$^i}$.
%=0.5~$, since according to our hypotheses ${{{c}}_{\$i}}$=1:
\begin{eqnarray*}
	S^i=\left( \frac{1}{1+{{c}_\$^i}}\right)*\left({S_\$^i}-{{C}_{\$}^i}\right)
\end{eqnarray*}
$S^i$ represents the overall impact of a shock on prices in each sector of each country expressed in the shocked currency. To transform this number in an average price index, $\bar{s}^i$, we need to compute a weighted average of the sectoral effects of the shock. This paper focuses of the household consumption price index.
...
$\overline{s_{i}}^{i,HC}$ provides the average impact of the shock on the consumer price of country $i$. 
 \begin{eqnarray}
\overline{s}_{i}^{i,HC}=\underset{\begin{subarray}{c}j=1\dots J \\ k=1 \dots I \end{subarray}}{\mathop \sum}\,{{s}_{kj}^i}.\frac{{hc}_{kj}^i}{hc^i}
\label{eq:eq6}
 \end{eqnarray} 
with ${s}_{kj}^i$ is a coefficient of $S^i$ for country $i$, ${hc}_{kj}^i$ the consumption of industry $j$ from country $k$ in country $i$ and $hc^i$ the total household consumption of country $i$. 
...
\begin{tabular}{c}
\includegraphics[width=6.0in, height=4.2in]{Comparaison_WIOD_TIVA_2011.png}\\
...
\begin{eqnarray}
\overline{s}_i^{i,HC}=\overline{s}_{i,imported}^{i,HC} + \overline{s}_{i,domestic}^{i,HC} \\
\overline{s}_{i,imported}^{i,HC} = \underset{\begin{subarray}{c}j=1 \dots J   \\ k=1 \dots I \\ k \neq i \end{subarray}}{\mathop \sum}\,{{s}_{kj}^i}.\frac{{hc}_{kj}^i}{hc^i}
\label{equ:share_of_imported}
 \end{eqnarray}

Imported final consumption plays the largest role in explaining the reaction of inflation as measured by $\frac{\overline{s}_{i,imported}^{i,HC}}{\overline{s}_{i}^{i,HC}}$ (see Figure \ref{fig:share_impt}). Among the sectors, services and "neig" ("non-energy industrial goods") play the largest role.
...
\begin{tabular}{c}
\includegraphics[width=6.0in, height=4.2in]{Share_sector_HC_WIOD_2014.png}\\
\end{tabular}
...
\begin{tabular}{c}
\includegraphics[width=6.0in, height=4.2in]{Int_HC_WIOD_2014_dom.png}\\
...
\begin{tabular}{c}
\includegraphics[width=6.0in, height=4.2in]{Int_HC_WIOD_2014_impt.png}\\
\floatfoot{Intensity is measured as the explained share of inflation change divided by the share in final consumption}
\end{tabular}
