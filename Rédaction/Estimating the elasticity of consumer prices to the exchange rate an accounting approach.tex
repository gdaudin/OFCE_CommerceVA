...
We analyze the elasticity of the household consumption expenditure (HCE) deflator to the exchange rate, using world input-output tables (WIOTs) from 1995 to 2019. 
...
Such heterogeneity mainly reflects differences in the foreign product content of consumption and intermediate products. 
Direct effects through imported consumption and intermediate products entering domestic production explain most of the transmission of exchange rate appreciation to domestic prices.
...
As WIOTs are data-demanding and available with a lag of several years, we extrapolate a reliable year- and country-specific estimate of HCE deflator's elasticity from 2015 onwards, using trade data and GDP statistics. 
...
%%%Data
%%In this paper, we analyze the impact of exchange rate variations on domestic consumer prices using several datasets covering most advanced and emerging economies, from 1995 to 2019. 
%%%Method
...
%The Leontief's production model (or Input-Output model, I-O hereinafter) analyzes the impact of a demand shock in a closed economy \citep{Leontief1951}. 
...
As a result, our estimates provide an upper bound for HCE deflator's elasticity compared to results from the ERPT literature. 
...
% vf je rajoute une couche sur les implications et le biais pour répndre à la question du referee
...
% VF je propose de le retirer pour limiter la confusion que souligne le referee
...
% je modifie pour être sûrs qu'on cite des périmètres comparables, en utilisant Colavecchio2020, et cela montre aussi que nos résultats sont en ligne
Furthermore, our results broadly concur with the recent literature.
For instance, \cite{Colavecchio2020}, using local projection techniques, find an ERPT into consumer prices ranging from 0.07 to 0.23 in the euro area.
%recent studies using various methodologies (VAR, dynamic simultaneous equations, Bayesian VAR, local projections, etc.) tend to obtain lower ERPT than ours, around 0.05 (see notably \cite{Hahn2003}, \cite{Comunale2017}, \cite{Colavecchio2020}).
...
%VF je supprime car je l'ai mis plus haut dans le pararaphe motivation/ breve explication de la démarche
%This accounting approach helps identify which countries and sectors are under pressure to adjust their prices when subject to an exchange rate variation.  
% je propose de supprimer car je le met plus haut ainsi que dans la partie data de manière plus approfondie
%The Leontief's production model (or Input-Output model, I-O hereinafter) analyzes the impact of a demand shock in a closed economy \citep{Leontief1951}. Using the %trade in value-added analysis reconciles international trade statistics with national I-O tables, thus allowing to extend Leontief's analysis to an international %context.\\
...
We show that countries are affected in proportion to their trading links with the appreciating-currency country. 
For illustration, we estimate the accounting impact of US dollar appreciation.\\
% Impact par secteur
Third, building on sectoral data, we examine which sectors experience higher spillovers from an appreciation of the domestic currency. 
...
%Decomposition et canaux de transmission
Fourth, we look into the determinants of HCE deflator's elasticity and the role of global value chains in the transmission of exchange rate appreciation. 
...
To address the data gap in WIOTs, we extrapolate HCE deflator's elasticity from 2015 onwards using up-to-date GDP statistics and trade data on consumption and intermediates.
We obtain a reliable estimate of HCE deflator's elasticity up to 2019.
Most of the existing literature provides average elasticity computed over a sample of several years. 
By contrast, our proxies based on up-to-date trade and GDP statistics allow us to track the evolution of HCE deflator's elasticity in a more up-to-date way, from one year to the next. \\
...
%This paper focuses on the role of global value chains in inflation dynamics. We analyze the impact of an exchange rate appreciation using a Cobb-Douglas, partial equilibrium framework.\\
...
%When value chains are global, an appreciation of the exchange rate aﬀects the household consumption expenditure deflator (HCE hereinafter) through distinct channels: \textit{i)} the prices of imported ﬁnal goods sold directly to domestic consumers;
...
%The construction of World Input-Output tables (WIOT hereinafter) is data-demanding and WIOTs are typically released with a lag of several years.
...
%Ancienne section 1 littérature
...
%\label{subsec:io}
%The Leontief's production model (or Input-Output model, I-O thereafter) analyzes the impact of a demand shock in a closed economy \citep{Leontief1951}. 
...
%A number of studies \citep{Hummels2001,Daudin2006,Daudin2011, DeBacker2012,Johnson2012,Koopman2014, Amador2015,Los2016,Miroudot2020} analyze the value added content %of world trade. 
...
%Using this framework, \cite{Cochard2016} analyze the effects of costs on prices ("cost-push inflation") and show a higher impact for more open countries.
...
In this paper, we use several multi-year WIOTs for the sake of robustness: \textit{i)} the World Input Output Database (WIOD), \textit{ii)} the multi-regional input-output table (MRIO) developed by the Asian Development Bank (ADB) and \textit{iii)} the Trade in Value Added Database (TiVA) from the OECD-ICIO.
...
However, the breakdown of these into domestic and imported products is not available. 
...
\section{An accounting approach to computing HCE deflator's elasticity}
\label{sec:metho}
...
Leontief's production model has a dual: the cost-push price model, which is useful in analyzing the consequences of changes in production prices.
...
%VF Je propose d'enelver car on a recentré le propos et on comprend moins la comparaison maintenant que tout est passé dans l'annexe B
%In both cases, implementing an exchange rate variation is more complex than implementing a change in production costs. 
The appreciation of the currency of country $A$ leads to a fall in the domestic currency price of country $A$'s imports, while the foreign-currency price of its exports increases. \\
...
Suppose a world with two countries $A$ and $B$, each having its own domestic currency, and using a third currency for international transactions, the dollar.
...
Country $B$ pays more for its imports of inputs, in dollars as well as in domestic currency, since its exchange rate against the dollar has not changed. 
Conversely, the prices of imported inputs in country $A$ in dollars increase since the prices of country $B$ have increased to accommodate more expensive inputs from country $A$. Prices in country $A$ in domestic currency decline however, as this effect is more than counter-balanced by the appreciation of country $A$'s domestic currency.
This has an impact on consumer prices expressed in domestic currency in country $A$.\footnote{For the sake of simplicity, we make the strong assumption that producers completely pass the exchange rate appreciation on to their production prices.  We do not consider other relevant determinants of the exchange rate pass-through. For instance, pricing-to-market strategies of exporters aiming to defend their market shares would imply a lower exchange rate pass-through.
Similarly, settlement and invoicing of imports in the domestic currency is another factor likely to weaken the elasticity of domestic prices to exchange rate movements. Assuming that only 60\% of invoices in the euro area goods trade are denominated in foreign currency (see \cite{Ortega2020}) would entail lower elasticity values. To anticipate on our results (see Figure \ref{fig:decompositionofs}), reducing the exchange rate pass-through on consumption goods would entail lower elasticity values than reducing the pass-through on inputs. Our estimates thus provide an upper bound for HCE deflator's elasticity.}\\
...
The overall impact of the exchange rate appreciation in dollar terms is equal, for the shocked country $A$, to the decline in consumer prices directly due to the exchange rate fluctuation, plus direct and indirect decreases (via inter-industry linkages in the country), in domestic currency and then converted back into dollar terms, in the prices of inputs imported from $B$ and disseminated to all branches. 
The overall impact on prices in dollar terms in country $A$ is therefore lower than the initial exchange rate variation, as domestic currency prices are also affected. 
...
\end{eqnarray}
\\
For the shocked country, currency appreciation has a disinflationary effect. In domestic currency, the prices of imported inputs fall in each sector by $c^i=-\frac{c_\$^i}{1+{c_\$^i}}$, or by $-\frac{10}{1.1}\%$ where $c_\$^i=10\%$.\\
...
This decline then spreads to all domestic-input using sectors. In sector $j$ of shocked country $i$, this fall amounts in domestic currency to: \\
\begin{eqnarray*}
...
Let $C_\$^i$ be the vector of log point changes in dollar prices following the 
%{100$\%$}
...
%Define ${{\tilde{C}}_{\$}}$ as the vector of change in input prices imported by country i, in dollars, $( -c_{\$i}\ldots0\ldots-c_{\$i})$. 
%
...
\noindent\fbox{%
	\parbox{\textwidth}{%
		...
		\begin{itemize}
		\item $a_{kl,ij}$: the share of inputs from country $i$'s sector $j$ in the production of country $k$'s sector $l$.
		...
		\item $c_\$^i$: change in the exchange rate of the currency of country $i$ in dollars. If it appreciates by 10\%, $c_\$^i = 0.1$.
		...
		$C^i = \left(-\frac{c_\$^i}{1+c_\$^i},\ldots0\ldots,-\frac{c_\$^i}{1+c_\$^i} \right)=\left(c_i,\ldots0\ldots,c_i \right)$ 
			\item 	$C_\$^i$: vector of log point changes in dollar prices following the 
		appreciation of the currency of country $i$ against all other currencies.
		$C_\$^i=\left(0\ldots0\ldots c_{\$ij}\ldots c_{\$ik}\dots 0\ldots0\right)$
		where $c_{\$ij}=c_{\$ik}=c_\$^i$
		...
		\item $p_{\${ij}}$: price of goods produced by sector $j$ in country $i$ in dollars (or any international reference currency)
	\end{itemize}
	}%
}
\\
The first-step impact of the appreciation of the country $i$’s currency on the price of its inputs corresponds, in domestic currency, to $C^i{\tilde{{\cal B}^i}}$ where: 
\begin{eqnarray}
...
We then convert the direct impact into dollars, by multiplying it by the new value of the domestic currency in dollars, $\left(1+c_\$^i\right)$. The direct impact of the appreciation of country $i$’s currency on the price of its inputs corresponds, in dollars, to ${\tilde{C}_\$^i}{\tilde{{\cal B}^i}}$ where: 
\begin{eqnarray}
...
\label{eq:eqinternationalcurrency}
 \end{eqnarray}
Where $S_\$^i$ is the total impact vector composed of the elements ${{{s}}^i_{\$kj}}$ showing the total impact of a change in the exchange rate of country $i$ on the price of country $k$'s sector $j$ in international currency expressed in log points. 
...
By contrast, we focus on the effect of an exchange rate variation on consumer prices. Hence, we are interested in the same impact expressed in domestic currency. To obtain the evolution of the sectoral prices of the shocked country in domestic currency, we remove the exchange rate variation in international currency, multiply the balance by the scalar of conversion equal to $\frac{1}{1+c_\$^i}$ and add the initial exchange rate variation in domestic currency.
...
Following an appreciation of the domestic currency versus all other currencies, imported inputs and imported consumer goods become cheaper and domestic prices expressed in domestic currency decrease. 
...
Using the WIOD, which covers a sample of 43 countries, we find that HCE deflator's elasticity ranges from 0.05 to 0.22 depending on the country (see Figure \ref{fig:WIOD_HC_elasticities}). 
...
Hence, pricing-to-market strategies of exporters aiming to defend their market shares would imply a lower exchange rate pass-through. 
As a result, our estimates provide an upper bound for HCE deflator's elasticity compared to other results in the literature.\\
In addition, we work under the producer pricing assumption. 
...
%For euro-area members, we distinguish between the effect of the appreciation of the euro and the effect of the appreciation of a hypothetical domestic currency. 
%For France, the elasticity of the HCE deflator to an appreciation of the euro is -0.08. 
%The elastiticy of the HCE deflator to an appreciation of an hypothetical French domestic currency is -0.122. 
...
\begin{figure}[H]
\centering
\caption{\footnotesize{\textbf{Comparison of HCE deflator's elasticity to the exchange rate for the WIOD, TiVA database rev. 3 and TiVA database rev. 4 (2011 and 2014)}}}
\begin{tabular}{c}
...
\begin{figure}[H]
	\centering
	\caption{\footnotesize{\textbf{HCE deflator's elasticity and share of imported consumption in total consumption (WIOD, 2014)}}}
	\begin{tabular}{c}
...
\begin{figure}[H]
	...
	\begin{tabular}{c}
		\includegraphics[clip, trim=0in 0in 0in 1.2in, width=4.5in, height=5.25in]{WIOD_HC_elasticities_dollar_appreciation.png}\\
		\floatfoot{Sources: WIOD and authors’ calculations.}
	\end{tabular}
...
\subsection{Evolution of HCE deflator's elasticity over time}\label{subsec:timeevol}
...
Using comprehensive measures of global value chain integration, \cite{Timmer2016} find that the expansion of global value chains has slowed since the Great Recession.\\
...
However, the version of the MRIO database we use (March 2021 release) suffers from data quality issues for 2018 and 2019.\footnote{Between 2017 and 2018, according to the MRIO database, the HCE deflator's elasticity sharply increased in a number of countries (e.g. China and India). See Online Appendix D for more details.} 
...
\begin{figure}[H]
	\centering
	\caption{\footnotesize{\textbf{Evolution of HCE deflator's elasticity, 1995-2019}}}
	\begin{tabular}{c}
...
%\begin{figure}[!h]
%	\centering
%	\caption{\footnotesize{\textbf{Consumer price elasticity and share of imported consumption computed with WIOD (2014)}}}
%	\begin{tabular}{c}
...
\subsection{Contributors to HCE deflator's elasticity}\label{subsec:contributors}
In this section, we analyze the contributions of each part of the consumption basket to the pass-through of exchange rate variations into consumer prices.
...
\begin{figure}[H]
	\centering
	\caption{\footnotesize{\textbf{Contribution of imported and domestic final goods and services to HCE deflator's elasticity (WIOD, 2014)}}}
	\begin{tabular}{c}
		\includegraphics[width=4.5in, height=3in]
...
We also examine the contributions of each sector to HCE deflator's elasticity.
...
\begin{figure}[H]
	\centering
	\caption{\footnotesize{\textbf{Contribution of different products to HCE deflator's elasticity (WIOD, 2014)}}}
	\begin{tabular}{c}
		\includegraphics[width=4.5in, height=3in]
		{decomp_sect.png}\\.
	\end{tabular}
...
\begin{figure}[H]
	\centering
	\caption{\footnotesize{\textbf{Contribution of domestic and imported components to HCE deflator's elasticity (WIOD, 2014)}}}
	\begin{tabular}{c}
		\includegraphics[width=4.5in, height=3in]{decomp_sectxorigin.png}\\
		\floatfoot{Sources: WIOD and authors’ calculations}.
	\end{tabular}
...
%
%Similar decompositions of the matrix $HC^i$ can be done to analyze the contribution of different sectors. decompose the effect between sectors. That allows the identification of the contribution of different sectors to the inflation shocks in different countries.
...
\section{Filling the data gap: estimating HCE deflator's elasticity without resorting to WIOTs}
\label{sec:extrapo}
\subsection{Assessing the role of global value chains in the transmission of exchange rate variations to domestic prices}
In this section, we analyze the determinants of HCE deflator's elasticity. 
...
Figure \ref{fig:shareofs} shows that input-output mechanisms (i.e. all channels except $E1.HC$) explain a large share of HCE deflator's elasticity, especially for large countries or euro area countries.
Figure \ref{fig:shareofsthroughtime} shows that input-output mechanisms have explained an ever increasing share of HCE deflator's elasticity since the early 2000s, implying a greater need for data from WIOTs to perform our computations.

\begin{figure}[H]
\centering
\caption{\footnotesize{\textbf{Breakdown of HCE deflator's elasticity into $E1.HC$, $E2.HC$, $E3.HC$ and $E4.HC$ (WIOD, 2014)}}}
...
\begin{figure}[H]
	\centering
	\caption{\footnotesize{\textbf{Role of input-output mechanisms in explaining HCE deflator's elasticity (WIOD, 2014)}}}
	%La figure vient de Étude rapport D+I et Bouclage Mondial_oil.do
	\begin{tabular}{c}
...
\begin{figure}[H]
	\centering
	\caption{\footnotesize{\textbf{Increasing role of input-output mechanisms in explaining HCE deflator's elasticity over time}}}
	%La figure vient de Étude rapport D+I et Bouclage Mondial_oil.do
	\begin{tabular}{c}
...
\subsection{Estimating HCE deflator's elasticity using the shares of imported goods and imported inputs in household consumption}
...
The significance of these two channels suggests that HCE deflator's elasticity could be estimated using national account data and input-output matrices. 
National account data provide $E1.HC$ and $E2.HC$, whereas world input-output matrices are needed for computing $E3.HC$ (effect on imported
consumer goods through domestic inputs) and $E4.HC$ (the residual). 
We investigate whether $E3.HC$ and $E4.HC$ can be inferred from easier-to-compute elements of HCE deflator's elasticity ($\overline{s}_{i}^{i,HC}$).
...
The high $R^2$ (0.98) suggests that $E1.HC+E2.HC$ is a good predictor of HCE deflator's elasticity ($\overline{s}_{i}^{i,HC}$). 
...
\begin{figure}[H]
\centering
\caption{\footnotesize{\textbf{Estimating HCE deflator's elasticity using the shares of imported goods and imported inputs in household consumption (WIOD, 2014)}}}
\begin{tabular}{c}
...
\begin{figure}[H]
\centering
\caption{\footnotesize{\textbf{Estimating HCE deflator's elasticity using the shares of imported goods and imported inputs in household consumption: evolution of the constant ($\alpha$), the coefficent of E1.HC+E2.HC ($\beta$) and $R^2$ over time}}}
\begin{tabular}{c}
...
Our results suggest that we can approximate HCE deflator's elasticity using the share of imported goods in household consumption and the share of imported inputs in household consumption of domestic goods. Indeed, $E1.HC+E2.HC$ is a good predictor of the total effects.\footnote{Interestingly, the total effects cannot be extrapolated in a multiplicative way, as the other effects ($E3.HC+E4.HC$) add to $E1.HC+E2.HC$ rather than amplifying them.
...
\subsection{Extrapolating HCE deflator's elasticity using GDP and trade statistics}\label{sec:Extrapolations}
...
We estimate HCE deflator's elasticity from 2015 onwards using GDP statistics and trade data on consumption and intermediates.\\
...
Hence, we cannot use them to extrapolate HCE deflator's elasticity.
...
\begin{figure}[!h]
	\centering
	\caption{\footnotesize{\textbf{Comparing HCE deflator's elasticity in 2014 (WIOD) and the prediction from a panel regression on the 2000-2008 period with fixed-effects}}}
	\begin{tabular}{c}
...
Hence, we can use the regression results presented in Table \ref{tab:reg_infer2} to predict HCE deflator's elasticity for the years for which WIOTs are not available, especially from 2015 onwards.
...
\begin{figure}[H]
	\centering
	\caption{\footnotesize{\textbf{Comparing output-weighted HCE deflator's elasticity computed using WIOTs to proxies based on up-to-date trade and GDP statistics.}}}
	\begin{tabular}{c}
		\includegraphics[width=5.0in, height=3.5in]{"predictions_reg1_doigt_mouille_trend_no".png}\\
	\end{tabular}
	\label{fig:panel_pred3}
		\floatfoot{Sources: WIOD, TIVA database, MRIO database, World Bank, BACI and authors’ calculations}
\end{figure}
...
We make several assumptions to simplify our computations.
As a result, our estimates provide an upper bound for HCE deflator's elasticity. \\
We make several contributions to the literature. \\
First, we analyze the composition and determinants of HCE deflator's elasticity using world input-output tables covering twenty-four years of data, from 1995 to 2019. 
...
Non-energy industrial goods explain the bulk of the total HCE deflator's elasticity. 
Services also play a significant role, especially in advanced economies such as the US, Japan, Germany and France.
...
Thus, even small price changes have a large effect on the HCE deflator. \\
Fourth, we analyze the determinants of HCE deflator's elasticity and the role of global value chains in the transmission of an exchange rate appreciation to domestic prices.
...
By contrast, the participation in global value chains plays a limited role. \\
...
We estimate HCE deflator's elasticity using GDP statistics and trade data on consumption and intermediates and obtain reliable out-of-sample predictions.
...
%To address this gap, we use more up-to-date GDP and trade data to approximate the HCE deflator's elasticity from 2016 onwards.
