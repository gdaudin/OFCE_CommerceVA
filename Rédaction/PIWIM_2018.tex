...
This is the objective of this paper: measuring the accounting effect of exchange rate fluctuations on consumer prices fully taking into account the existence global value chains over twenty years (from 1995 to 2005).
...
An additional contribution of this paper is to that the findings of this data-hungry exercise can be extrapolated through time up to relatively data-poor recent periods.
...
\cite{Gaulier2019} show the intermediate trade share in volume grew at a subdued rate between 2000 and 2016, while the share of intermediate goods in world trade in nominal terms was fairly well correlated to various Global Value Chain indicators based on international input-output matrices.  
\cite{DeSoyres2018} find the exchange rate pass through decreases as the foreign value added increases using a sector-country of origin-country of destination-level panel regression for the pricing behaviour of exporters. 
\cite{Georgiadis2019}  find a structural two-country model with trade in intermediate goods and staggered price setting that the size of the exchange rate pass through depends is the integration in global value chains not only of a country, but also of its trading partners.
Nevertheless, \cite{Leigh2017} find limited evidence that participation in global value chains has significantly changed the exchange rate–trade relationship over time.
...
Concerning the evolution of the pass through over time,  \cite{Campa2008} have documented a fall in ERPT to import and consumer prices since the 1980s and 90s. \cite{Ozyurt2016} shows that the pass through is only “partial” in the euro area with clear evidence that the degree of pass through has been declining over the past two decades. In particular, she finds that the period of strong fall of pass through coincides with the increasing share of the emerging countries in world trade and the accession of China to the WTO. Concerning potential non-linearities, \cite{Colavecchio2020} find that euro appreciations and depreciations affect the level of euro area exchange rate pass-through in a symmetric fashion, but results for euro area countries differ for import and consumer prices and across countries. Concerning import prices, \cite{Bussiere2016} find no significant difference in the import pass-through between emerging and advanced economies.
