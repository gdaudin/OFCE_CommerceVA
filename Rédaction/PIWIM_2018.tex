...
The TiVA database is compiled by the OECD (initially associated with the WTO, but not for the latter versions). It builds on the OECD harmonised individual country input-output tables to provide matrices of inter-industrial flows of goods and services in current prices (U.S. dollars).
...
For example, both start with the construction of harmonized country-specific supply-use tables (SUTs) that are then transformed into world input-output tables.
...
Another difference is that in harmonizing national data and trade statistics, WIOD treats the shares of imported inputs given by national account as given, and TiVA does not. As a result, TiVA is closer to international trade statistics than WIOD. That explains why the share of imported inputs is smaller in TiVA for some countries (but not China and the USA).
...
The Houshold consumption expenditure deflator elasticity to a shock on the domestic currency ranges from 0.05 to 0.35, depending mainly on the openness of countries.
