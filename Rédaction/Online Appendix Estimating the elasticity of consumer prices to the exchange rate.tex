...
\begin{document}
\title{Estimating the elasticity of consumer prices to the exchange rate: an accounting approach \\ Online Appendix	\thanks{ All programs are available at \url{https://github.com/gdaudin/2023 JIMF Camatte Daudin Faubert Rifflart}. Data and results are available upon request.}\\
\vspace{1cm}
}
\vspace{1cm}
\date{\today}
...
\newpage
\section*{Appendix B: Defining the impact of a change in prices using an I-O model}\label{subsec:ioprice}
In this appendix we compute the well-know effect of the impact of a change in prices using an I-O model.
...
In addition, firms face a change in their costs, which affects their prices according to equation 1 in the main text.
...
\newpage
\section*{Appendix C: Comparing the impact of a change in prices with that of an exchange rate variation}
...
\newpage
\section*{Appendix D: MRIO data quality issues}

Figure \ref{fig:panel_pred3} shows that the HCE deflator's elasticity extrapolated from WIOD, World Bank and BACI data for the period 2015-2019 is close to the elasticity obtained with MRIO in 2016-2017. By contrast, the elasticity estimated using MRIO is much higher in 2018 and 2019. 
...
Figure \ref{fig:pays_soucis_MRIO} shows that the HCE deflator's elasticity estimated using MRIO increased by more than 20\% for India and China between 2017 and 2018.
...
\begin{figure}[H]
	\centering
	\caption{\footnotesize{\textbf{Comparing the output-weighed HCE deflator's elasticity to predictions based on World Bank and BACI data.}}}
	\begin{tabular}{c}
...
\begin{figure}[H]
	\centering
	\caption{\footnotesize{\textbf{Comparing country-specific HCE deflator's elasticities computed using MRIO in 2016, 2017 and 2018.}}}
	\begin{tabular}{c}
...
\begin{figure}[H]
	\centering
	\caption{\footnotesize{\textbf{Comparing country-specific HCE deflator's elasticities computed using WIOD in 2012, 2013 and 2014.}}}
	\begin{tabular}{c}
...
\newpage
\section*{Appendix E: Estimating the HCE deflator's elasticity using the shares of imported consumption and imported intermediates in domestic consumption} \label{AppendixFonctionLinéaireTIVA}

As a complement to section 4.2, Figures \ref{fig:ratiodir_TiVA} and \ref{fig:evolution_coef_TiVA} for TIVA rev. 3 and Figures \ref{fig:ratiodir_TiVA_REV4} and \ref{fig:evolution_coef_TiVA_REV4} for TIVA rev. 4 show that using the share of imported final consumer goods and services and the share of imported intermediate goods in domestic final consumption provides a good prediction of the elasticity of the HCE deflator to the exchange rate.
...
\newpage
\section*{Appendix F: Transmission channels of an exchange rate variation in the two-country, one-sector case}\label{AnalyticalAppendix}
...
Figure \ref{fig:tous_les_E} confirms that, in that numerical exercise, the total effect is dominated by the direct effect through imported consumer goods and, to a lesser extent, the effect on domestic consumer goods through imported inputs. 
...
\newpage
\section*{Appendix G: Extrapolating the HCE deflator's elasticity using household consumption, intermediate consumption and trade statistics}  % je n'avais pas propose un titre plus clair dans une precedent version?
Section 4.2 shows that the sum of the share of imported goods in household consumption and the share of imported inputs in household consumption of domestic goods ($E1.HC + E2.HC$) is a good predictor of the HCE deflator's elasticity to the exchange rate. 
...
Section 4.3 shows that trade and GDP data are a good predictor of the HCE deflator's elasticity.
...
Combining these three data sources (BACI, World Bank and Eurostat), we compute the share of imported consumer goods in household consumption and the share of imported inputs in all inputs. 
...
%\begin{equation}
%	\begin{array}{llll}
%		\overline{s}_{i}^{i,HC}= \alpha &+  \beta_1  \frac{\textnormal{imported consumer goods}_i}{\textnormal{household consumption}_i} \\ & +  \beta_2  \left[    \frac{\textnormal{imported intermediate goods}_i}{\textnormal{intermediate consumption}_i}*\frac{\textnormal{domestic consumer goods}_i}{\textnormal{household consumption}_i}\right] +\varepsilon_i 
%	\end{array} 
%	\label{eq:eq8}
%\end{equation}
%
...
%Estimating successive cross-sections of equation \ref{eq:eq8} is a demanding test. 
%In addition, these regressions fail to account for country-specific determinants of the HCE deflator's elasticity.
...
\begin{equation}
	\begin{array}{llll}
		\overline{s}_{i,t}^{i,t,HC}= &  \beta_1  \frac{\textnormal{imported consumer goods}_{i,t}}{\textnormal{household consumption}_{i,t}} \\
		& + \beta_2 \left[ \frac{\textnormal{imported intermediate goods}_{i,t}}{\textnormal{intermediate consumption}_{i,t}}*\frac{\textnormal{domestic consumer goods}_{i,t}}{\textnormal{houshold consumption}_{i,t}}\right] \\ 
		&+  \beta_3 \frac{\textnormal{Total imported consumer goods}_t}{\textnormal{Total household consumption}_t} \\
		& + \beta_4  \left[ \frac{\textnormal{Total imported intermediate goods}_t}{\textnormal{Total intermediate consumption}_t}*\frac{\textnormal{Total domestic consumer goods}_t}{\textnormal{Total household consumption}_t} \right] \\ 
...
\begin{figure}[H]
	\centering
	\caption{\footnotesize{\textbf{Comparing the HCE deflator's elasticity in 2014 (WIOD) and the prediction from a panel regression on the 2000-2008 period with fixed effects}}}
	\begin{tabular}{c}
