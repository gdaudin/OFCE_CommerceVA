...
We study here the effect on consumption prices of exchange rate shocks.
By using three sectoral world input-output datasets, we find that the contribution of first-round effects on imported final and intermediate goods in domestic consumption, explains the bulk - around three-quarters -  of the propagation of global shocks to domestic prices.
We also show that the first-round effects, through imported consumption and imported inputs to domestically produced consumption goods, are a good predictor of the total effects.
Finally, we establish that the results obtained using sectoral world input-output datastests up to 2015 can be extrapolated to recent years thanks to the use of easily obtainable statistics as GDP and trade statistics.
...
This has to be taken into account to study international interdependence.
For example, in the absence of global value chains, the only way currency appreciation  affects consumer prices is through a reduction in the price of imported consumption goods.
But if there are global value chain, an other channel  opens.
Currency appreciation reduces the domestic price of imported inputs, and thus further reduces consumer prices.
The other way around, because of exported intermediate goods, inputs become more expensive for foreign firms.
That increases their production costs and, presumably, also increases domestic consumer prices in the country which currency appreciates.
The changes in domestic and foreign production costs will in turn lead to changes in the price of inputs for domestic and foreign firms and cause further production costs variations.
Of course, the behavior of firms is decisive to determine the effect of exchange rate shocks on prices.
Still, working out the full potential accounting effect can help understand what kind of pressure the existence of global value chains subjects firms to.

This is the objective of this paper: measuring the accounting effect of exchange rate fluctuations on consumer prices fully taking into account the existence global value chains over twenty years (from 1995 to 2015).
Most of the literature of global supply chains focuses on the role of input-output linkages in propagating shocks to production.
In contrast, we examine the role of input-output linkages in propagating shocks to consumer prices.
We analyze how sectoral inflation reacts to global inflationary shocks by examining the main components of consumer prices (manufacturing goods, services, food and energy on the one hand, imported and domestic goods on the othre.). 
We provide evidence that, following an appreciation of the domestic currency, the direct effect of global inflationary shocks, i.e. the contribution of first-round effects on imported final and intermediate goods in domestic consumption, explains the bulk of the propagation of global shocks to domestic prices. By contrast, we find a limited role for the additional transmission of lower domestic input prices to other sectors of the domestic economy and other countries occurring during subsequent production cycles.
This limited role is the same in absolute terms for most countries.
We also show that the results obtained from three different sources (two versions of TiVA and WIOD) are very similar.

This is a particularly data demanding exercise because it requires World Input-Output tables (WIOT hereafter) that are difficult to construct.
This can be a major obstacle for taking into account global value chains in inflation forecasts. 
...
The remainder of the paper is organized as follows. Section \ref{sec:lit} briefly describes the related literature. Section \ref{sec:metho} presents the methodology and the data sources.  Section \ref{sec:prixconso} computes the accounting impact of an exchange rate shock on consumer prices up to 2015.  Section \ref{sec:extrapo} shows that, combined with findings of section \ref{sec:prixconso}, GDP and trade data are all that is needed to compute the accounting impact of an exchange rate shock on consumer prices.This means it is possible to use our results to examine what happened during the second half of the 2010s.
...
\cite{Gaulier2019} show the intermediate trade share in volume grew at a subdued rate between 2000 and 2016, while the share of intermediate goods in world trade in nominal terms was fairly well correlated to various Global Value Chain indicators based on international input-output matrices. \cite{AntoundeAlmeida2016} also show that the cross-border sectors pairs which trade more intensively with each other in intermediate inputs display higher PPI inflation correlation, indicating  price spillovers along the global supply chain. 

\cite{DeSoyres2018} found the exchange rate pass through decreases as the foreign value added increases using a sector-country of origin-country of destination-level panel regression for the pricing behaviour of exporters. In particular, they found that the ERPT to France and Germany declines most, whereas the impacts are small for most countries. \cite{Georgiadis2019} also show that an increase in the economy’s global value chain participation implies a fall in its trading partner’s exchange rate pass-through to import prices. Their empirical estimation implies that the rise in global value chain participation can account for about 50\% of the decline in exchange rate pass-through to import prices since the mid-1990s. We can also mention that \cite{Hagemejer2020} found that ERPT for suppliers strongly involved in the GVC production is four times smaller than for economies not participating in the GVC. 

Concerning the cross border propagation of cost shocks, \cite{Auer2017} document that input-output linkages contributes substantially to synchronizing producer price inflation across countries, combining data on sectoral domestic and international input trade from the World Input Output Database (WIOD) and producer price indices. By contrast, exchange rate movements and the degree of pricing-to-market are found to play no role in synchronizing inflation across countries. 
...
The empirical literature proves the pass through declines across the pricing chain. An ECB report \cite{Ortega2020} confirms the pass through is highest and fastest for import prices at the border, but significantly smaller and relatively slower for final consumer prices (see also \cite{Hahn2003, Kunovac2017,BenCheikh2017}

In an input-output accounting perspective, the exchange rate pass through to consumer prices is bounded by the direct and indirect import content of consumption. According to calculations based on the World Input-Output Database (WIOD),  \cite{Schaeffer2019} (box 1 Import share in the HICP consumption basket) finds that the sum of the direct and indirect import content in consumption for the euro area was 16 per cent in the HICP and 14 per cent in the HICP excluding energy and food, HICPX, in 2014 under the assumption of full pass-through to import prices.

The question about a potential change of the pass through over time remains very discussed in the literature. On the one hand, \cite{Campa2008} have documented a fall in ERPT to import and consumer prices since the 1980s and 90s. \cite{Ozyurt2016} also shows that the pass through is only “partial” in the euro area with clear evidence that the degree of pass through has been declining over the past two decades. In particular, she found that the period of strong fall of pass through coincides with the increasing share of the emerging countries in world trade and the accession of China to the WTO. On the other hand, \cite{Leigh2017}, by using three alternative approaches, found that both exchange rate pass-through and the price elasticity of trade volumes are largely stable over time (for them, economic slack and financial conditions affect the exchange rates and trade relationship). In the euro area, time-varying estimates (see \cite{Ortega2020}) show the ERPT to total import prices has been broadly stable over time (at around 20 \%), but with a decline of exchange rate pass through to extra-euro area import prices 

Concerning potential non-linearities, the price response to depreciations and appreciations may differ depending on the firm’s market share strategy, partly determined by the competitive structure of the sector and market power of firms (\cite{Delatte2012}). \cite{BenCheikh2012} finds a higher ERPT to consumer prices after large exchange rate changes. In France and Germany, \cite{Bussiere2013} shows the ERPT tends to be higher after depreciations (and/or strong depreciations) than after appreciations for import prices. Last but not least, more recent work by \cite{Colavecchio2020} on euro area countries found that after one year, large changes in the exchange rate have an impact on import prices and headline HICP inflation, whereas small ones do not. 
...
In France, the Insee \citep{Bourgeois2019} has developed the AVIONIC model based on French symmetric input-output tables that measures the effect on the price of production of an exogenous variation on the price of inputs. 
