...
As WIOT are not available for the latest years, we estimate the HCE elasticity from 2015 onwards using trade data on consumption and intermediates and GDP statistics without resorting to WIOT. 
We show that a precise assessment of the HCE elasticity to exchange rate can be estimated without resorting to WIOT. 
...
%Data
We analyse the composition and determinants of the HCE elasticity using world input-output tables (WIOT hereafter) covering twenty years of data (from 1995 to 2015).
For the sake of robustness, we rely on several datasets (WIOD and two distinct releases of the OECD TIVA database). 
%Method
We perform an accounting exercise based on information contained in the WIOT with large matrices inversion. 
We make two additional assumptions to simplify the computations. 
We assume that the exchange rate shock is proportional across sectors and assume a full exchange rate pass-through to import prices. 
...
These can be considered an upper bound as the elasticity is estimated under the symplifying assummption of full exchange rate pass-through to import prices and under the  producer pricing assumption. Alternative pricing assumptions (e.g. pricing-to-market) would entail even lower elasticity values.  
\\
The elasticity of the HCE deflator to the exchange rate has remained broadly stable over the past two decades.
Using the WIOD dataset, which covers a sample of 43 countries, we find that the mean output-weighted elasticity of the HCE deflator increased from 0.08 in 2000 to 0.09 in 2008. 
...
%Extrapolation
The construction of World Input-Output tables is data-demanding and WIOTs are typically released with a lag of several years.
As a result, WIOT are not availbale for the latest years. The most recent dataset available dates back to 2015. 
...
By doing so, we show that a precise assessment of the HCE elasticity to the exchange rate can be estimated without resorting to WIOT. \\
% Plan
