...
We build on three sectoral world input-output datasets (WIOD and two versions of TiVA). We assume Cobb-Douglas production framework and work in a partial equilibrium setting.
...
Depending on countries, the absolute value of the elasticity of the household consumption expenditure (HCE) deflator to the exchange rate ranges from 5\% to 35\%, confirming the importance of global value chains in channeling external shocks to domestic inflation.
...
Using data from WIOD on a sample of 43 countries, we find that the mean output-weighted elasticity of the HCE deflator to the exchange rate increased in absolute value from 7.5\% in 2000 to 9.4\% in 2008. 
...
The “missing inflation puzzle" emerged from 2012 onwards, as inflation remained subdued despite the economic recovery (see \cite{Ball2011, Coibion2015}). 
From a central bank perspective, the underlying question is whether the Phillips curve is a reliable tool to explain inflation in a globalised world.
...
All agree that external factors play a role in driving domestic inflation, either through more integration or more coordination.
...
When value chains are global, fluctuations in exchange rates (we assume here an appreciation of a currency) aﬀect household consumption deflator (HCE hereafter) through distinct channels: \textit{i)} the prices of imported ﬁnal goods sold directly to domestic consumers;
...
We measure the partial equilibrium effect of exchange rate fluctuations on consumer prices, because the stabilisation of the consumer prices is a widespread policy objective for monetary authorities. We use world input-output tables covering twenty years of data (from 1995 to 2015).\\
...
The absolute value of the elasticity of the HCE deflator to exchange rate shocks varies widely between countries, from 5\% to 35\%. 
In the euro area, the elasticity of the HCE delfator to changes in the value of the euro ranges from 6.5\% to 18\%. 
Such heterogeneity add to the challenges faced by the European Central Bank in stabilising prices in the monetary union. 
In a sample of 43 countries, the mean output-weighted elasticity of the HCE deflator to exchange rate shocks based on WIOD increased in absolute value from 7.5\% in 2000 to 9.4\% in 2008.
...
Section \ref{sec:prixconso} computes the partial equilibrium impact of an exchange rate shock on HPE deflator up to 2015. 
Section \ref{sec:extrapo} relies on up-to-date GDP and trade data to approximate the partial equilibrium impact of an exchange rate shock on HPE deflator from 2016 onwards.
...
with ${\left( I-{\cal A} \right)}^{-1}$ the well-known of Leontief inverse matrix. If the shock is small enough, the elements of $S$ can be understood as the elasticity of sectoral price to an input price shock. Using consumption shares to do a weighted average of these elasticities will generate the Household consumption expenditure (HCE) deflator elasticity to an input price shock \footnote{This HCE deflator has different sectoral weights than the headline Consumer price inflation (CPI) as it also covers, for exemple, the rental equivalent of real estate expenditures}.
...
Using WIOD, we find that the absolute value of the elasticity of the HCE deflator to an exchange rate shock is 0.055 (or 5.5\%) for the US in 2011.
...
Using a sample of 43 countries, we plot the elasticity of the HCE deflator to the exchange rate over time. The anual evolution is the same regardless of the database (WIOD or TIVA) (see Figure \ref{fig:PIWIM_LONGITUDINAL}).
...
\begin{figure}[!h]
	\centering
	\caption{\footnotesize{\textbf{Distribution of the HCE deflator elasticity to a 100\% exchange rate appreciation (WIOD) - 2014.}}}
	\begin{tabular}{c}
...
The Houshold consumption expenditure (HCE) deflator elasticity to a shock on the domestic currency ranges from 5\% to 35\%, depending mainly on the openness of countries.
...
We thus provide a tool for approximating the HCE deflator elasticty to an exchange rate shock from 2015 onwards.
