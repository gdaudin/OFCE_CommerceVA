...
We can go even further and notice that requireng intermediate consumption and household production in our computations dramatically reduces the avaibility of the data. One could imagine an even simpler proxy for E1HC+E2HC that would require only trade data from Comtrade and GDP data from the world bank. We can then run another panel (see see equation \ref{eq:eq10})
...
Actual openess rates in the sample vary between 0.15 and 0.5. In that zone, the relationship between the openness rate and the residual is not monotonous, and actually does not vary much (see Figure \ref{fig:resid}).

Figure \ref{fig:tous_les_E} comfirmes that, in that numerical exercice, the total effect is dominated by the direct effect through imported consumption goods and, to a lesser extent, the effecto on domestic consumption goods through imported inputs. The other effects are approximately additive if the openess rate is betweem 0.15 and 0.5.
