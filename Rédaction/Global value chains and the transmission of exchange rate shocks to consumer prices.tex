...
%littérature \\
A large body of literature – from macro trade to input/output linkages - documents the exchange rate pass-through to domestic prices (ERPT hereafter). 
...
Our paper uses and contributes to both strands of literature as mechanisms are based on input/output matrix, but our results are comparable with macro-trade estimations, with the advantage of having the evolution of the ERPT for each year instead of having a unique elasticity for a given sample. \\
%We assume a Cobb-Douglas production framework where firms have a simple cost-minimising behaviour and work in a partial equilibrium setting.
