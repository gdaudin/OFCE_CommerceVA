...
Before looking into the channels, we look into how much each part of the consumption basket contributes to the pass-through of exchange rate variations to consumer prices.
We start with the contribution of domestic versus imported goods.
...
Although imported final consumer goods account for a smaller share of total consumption than domestic goods, it is quite natural that they are the most impacted by exchange rate fluctuations.\\
...
We examine now the industrial contributions of each sectors.
We regroup industries into four groupings: manufacturing goods, services, food and energy. 
...
Although services are mainly produced domestically and do not rely much on imported inputs, they represent a substantial share of total consumption: even small price changes have large impacts on the HCE. \\ 
...
We now want to analyse the determinants of the HCE elasticity and particularly to asses the role of global value chains in the transmission of an exchange rate appreciation to the HCE deflator. 
...
Hence, we can write, defining $HC^{i,dom}$ and $HC^{i,imp}$ as the domestic and imported shares of $HC^i$ and adjusting the dimension of $E1$, $E2$ and $E3$:
...
When the domestic currency appreciates, $E1.HC^{i,imp}$ ($E1.HC$ hereafter) and $E2.HC^{i,dom}$ ($E2.HC$ hereafter) reduce the consumer prices in country $i$ whereas $E3.HC^{i,imp}$ ($E3.HC$) increases them. 
...
While $E3.HC$ is negligible (except for Germany and the Netherlands), $E4.HC$ accounts for 10\% to 30\% of $\overline{s}_{i}^{i,HC}$ for most countries (more for India, Brazil, Portugal and Luxembourg). With the exception of Luxembourg, the less open to trade is a country, the larger the share of $E4.HC$. \\
...
The importance of $E1.HC$ and $E2.HC$ suggests that there might be a way to measure the HCE deflator elasticity to the exchange rates from national accounts and input-output matrices.
These provide  $E1.HC$ and $E2.HC$, whereas world input-output matrices are needed for computing $E3.HC$ and $E4.HC$. 
Maybe $E3.HC$ and $E4.HC$ can be inferred from easier-to-compute elements of $\overline{s}_{i}^{i,HC}$.
...
A good starting point might be that the sum of the share of imported goods in household consumption and the share of imported inputs in household consumption of domestic goods ($E1.HC + E2.HC$) are a good predictor of the impact of exchange rate fluctuations on household consumption prices. 
...
We do an out-of-sample prediction for WIOD, 2014, using data for years 2000 to 2008.
The results are satisfactory, although the mean and median errors are larger than in Onliine Appendix C (see Figure \ref{fig:panel_pred2}). 
Our findings are robust to using other databases (revision 3 and revision 4 of TIVA Results available upon request).
...
Second, we examine which part of the consumption basket most contributes to the elasticity of the HCE deflator to exchange rate variations. 
...
On the whole, virect effects through imported consumption and intermediates entering domestic production explain three-quarters of the transmission of an exchange rate appreciation to domestic prices.
By contrast, global value chains participation plays a limited role. \\
Fourth, we show that a precise assessment of the HCE deflator elasticity to the exchange rate can be extrapolated for years lacking widely distributed WIOTs.
Estimating the HCE deflator elasticity using GDP statistics and trade data on consumption and intermediates is possible an provides reliable out of sample predictions.
We hope this study will help explain and improve the variety of inflation elasticities to the exchange rate used in forcasting models.
