...
Despite these simplifying assumptions, our estimates provide an accounting-based gauge of how large an effect could have exchange rate variations on consumer prices, considering direct and indirect import content in consumption and global value chain linkages. 
...
%Extrapolation
Fourth, we show that a precise assessment of the HCE deflator elasticity to the exchange rate can be estimated for recent years without reliable WIOTs. 
The construction of World Input-Output tables is data-demanding and WIOTs are typically released with a lag of several years.
As a result, WIOT are not available for the most recent years. For instance, the latest WIOD dataset dates back to 2014. 
To address this gap, we extrapolate the HCE deflator elasticity from 2015 onwards using GDP statistics and trade data on consumption and intermediates. up-to-date GDP and trade statistics up to 2019.
This yields an estimate of the elasticity very close to the one computed from MRIO WIOT.\\
% Plan
...
All agree that the pass-through differs between countries, across sectors and over time. 
...
To address the analytical needs related to the Asian and Pacific regions, the Asian Development Bank (ADB) has augmented the WIOD with details for nineteen additional Asian economies\footnote{The MRIO database includes the following additional economies: Bangladesh, Bhutan, Brunei Darussalam, Cambodia, Fiji, Hong Kong, Kazakhstan, Kyrgyz Republic, Lao People’s Democratic Republic, Malaysia, Maldives, Mongolia, Nepal, Pakistan, Philippines, Singapore, Sri Lanka, Thailand and Viet Nam.}. 
This wider geographical coverage comes at the price of a less precise industrial desagregation (35 vs 56 in WIOD).
The conceptual framework of the ADB MRIO database (and hence the methods used in the construction of the tables) are similar to those of WIOD.
Its main advantage is of updated up to 2019.  
...
This assumption can be misleading. \cite{Feenstra2012} find that shares of imported materials may differ substantially across U.S. industries. Based on Asian input-output tables, \cite{Puzzello2012} finds that the proportionality assumption understates the use of foreign intermediate inputs. All databases, however, rely on the BEC (Broad Economic Categories) classification to map detailed six-digit products into intermediate use, final consumption and investment \citep{Dietzenbacher2013}. \\
...
Yet, to ease exposition we had to use one of them as a baseline. We have decided to use WIOD as it is the one with the most industries.

\subsection{Accounting impact of an exchange rate variation on domestic prices} \label{subsec:chocchange}
We are primarily interested in the impact of an exchange rate appreciation on consumer prices in country whose currency’s value is changing (country $A$ by convention). 
We also estimate the inflationary impact on countries that directly and indirectly, through third countries linkages, consume inputs from country $A$ and will present results on the effect of dollar variations.
...
Let $HC^i$ be the vector of sectoral shares in country $i$'s household consumption\footnote{Consumption is at market prices, whereas the WIOTs are all at basic prices. To move from one to the other, we make the simplifying assumption that all taxes and subsidies on products are proportional, as is the value-added tax.}:
\begin{eqnarray*}
HC^i=\left( 
	\begin{array}{c}
	\frac{{hc}_{11}^i}{hc^i} \\
...
The model can also track the effect of variations in the currency of other countries.
For illustration, we estimate the impact of an appreciation of the US dollar (USD) using WIOD.
...
However, the version of MRIO we use (March 2021) suffers from data quality issues for 2018 and 2019 (see Online Appendix F). Between 2017 and 2018, the HCE deflator elasticity sharply increased in a number of countries (e.g. China
...
and India). 
Hence, we assume that the elasticity estimated with MRIO is not reliable for 2018 and 2019.
Section \ref{sec:Extrapolations} gives more arguments on that point.\\
