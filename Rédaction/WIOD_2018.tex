...
To go further, $\overline{s_{i}}^{i,HC}$ is broken down by origin: imported final consumption goods on the one hand and domestic final consumption goods on the other hand. The price of domestic goods is affected because of imported inputs. E.g., we can define 
\begin{eqnarray}
...
We can then define the contribution of imported consumption to an exchange rate shock as $\nicefrac{\overline{s}_{i,imp}^{i,HC}}{\overline{s}_{i}^{i,HC}} = \nicefrac{S^i.HC^{i,imp}}{S^i.HC^{i}}$. We also define the sensitivity of imported consumption as $ \frac{\nicefrac{S^i.HC^{i,imp}}{S^i.HC^{i}}}{\vec{1}.HC^i,imp}$ where $\vec 1$ is a horizontal vector of ones.

GD : PEUT-ÊTRE Y A-T-IL UNE MANIÈRE DE SIMPLIFIER L'EXPRESSION DE LA SENSITIVITÉ, MAIS JE NE VOIS PAS COMMENT FAIRE...

Figure \ref{fig:share_impt} shows, for example, that the contribution of imported consumption is higher than 0.5 in most countries. While imported consumption has a smaller share in total consumption, this is overcompensated by the sensitivity of its prices to exchange rate shocks, as shown by the comparison of \ref{fig:intensity_dom} and \ref{fig:intensity_impt}.

Similarly, the matrix $HC^i$ is broken down to analyse the contribution of different sectors to inflation shocks in different countries.
...
\floatfoot{Interpretation note: the contribution of energy final consumption to the impact of a nominal exchange rate shock is lower than 15\% in most countries}
\end{tabular}
...
One might wonder whether using world input-ouput matrices provided by WIOD or TIVA is worth the wait (the latest version of WIOD covers 2014) and the additional complexity. To answer this question, we break down $\overline{s_{i}}^{i,HC}$ into different elements classified by ease of use and computation. Let us start from equation \ref{eq:eqdomcurrency}.
...
When the shock corresponds to an appreciation of the domestic currency, $E1$ and $E2$ reduce domestic prices and $E3$ increases domestic prices. Notice that $E1$ and $E2$ are easy to compute with national input-output matrices, whereas world input-output matrices are needed for computing $E3$ and $E4$.
...
Figure \ref{fig:ratiodir} depicts the relationship between $\overline{s}_{i}^{i,HC}$ and $E1.HC^{i,imp}+E2.HC^{i,dom}$. It shows that $E1.HC^{i,imp}+E2.HC^{i,dom}$ is a pretty good predictor of $\overline{s}_{i}^{i,HC}$ (the $R^2$ is over 0.95). What is surprising is that $E3.HC^i+E4.HC^i$ is constant accross countries. 
More formally, when we estimate equation \ref{eq:eq7}, we find that $\alpha$ is different from 0 and that $\beta$ is not significantly different from one (GD : À VÉRIFIER).
