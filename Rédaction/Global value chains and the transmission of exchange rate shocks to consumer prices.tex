...
As an example, we estimate the impact of an appreciation of the US dollar (USD) on the US trading partners using WIOD.
...
Figure \ref{fig:shareofs} shows that input-output mechanisms (i.e. all shares except $E1.HC$) explain a large share of the elasticity, especially for large countries or euro area countries.
...
Our predictions are in line with results from MRIO up to 2017. For the most recent years, there seems to be data issues in MRIO (see Online Appendix F), which the comparison helps to identify.
