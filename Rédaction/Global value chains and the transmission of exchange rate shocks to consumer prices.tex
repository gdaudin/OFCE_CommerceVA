...
The elasticity is stable over time but heterogeneous across countries (ranging from 0.05 to 0.22), reflecting different degrees of openness to trade and differences in foreign product content in domestic consumption. 
...
It depends, among other things, on trade openness, integration in international production chains, firms' pricing strategies and the currency of invoicing for trade. \\
...
Supply-driven price models have a number of well-documented limitations \citep{Folloni1994}.
These models rely on a number of simplifying assumptions.
...
The higher their reliance on those inputs, the higher the change in prices.\\
...
In a second step, the impact of the change in input prices is passed on to all the country-specific industries that rely on these inputs in their production processes. 
...
If the change in the exchange rate is small enough, the elements of $S$ can be understood as the elasticity of sectoral prices to a change in input prices. 
Using consumption shares to perform a weighted average of these elasticities, we compute the household consumption expenditure (HCE) deflator elasticity to a change in input prices \footnote{The HCE deflator has different sectoral weights than the headline consumer price index (CPI) as it also encompasses, for example, the rental equivalent of real estate expenditures.}.
...
This paper uses several multi-year WIOTs for the sake of robustness: the World Input Output Database (WIOD), the MRIO database developed by the Asian Development Bank and the Trade in Value Added Database (TiVA) from the OECD-ICIO.
...
The WIOD database provides disaggregate information for a limited number of Asian economies. 
...
For the sake of simplicity, we assume a full exchange rate pass-through. 
Producers are assumed to completely pass the exchange rate appreciation on their production prices.  
Hence, we do not consider other relevant determinants of the exchange rate pass-through.
...
Let us first look at the impact of the exchange rate variation from the perspective of countries that import inputs from country $i$.\\
...
$S^i$ represents the overall impact of a exchange rate variation on prices in each sector of each country expressed in the currency of country $i$. It is expressed in log points. If the variation is small enough, its elements correspond to the elasticities to the exchange rate of consumer prices in country $i$.
...
$\overline{s_{i}}^{i,HC}$ provides the HCE deflator of country $i$ elasticity to an exchange rate variation. 
 \begin{eqnarray}
...
\section{Accounting impact of a change in the nominal exchange rate on consumer prices}
\label{sec:prixconso}
