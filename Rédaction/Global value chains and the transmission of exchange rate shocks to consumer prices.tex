...
A first strand of literature documents the exchange rate pass-through to domestic prices (ERPT hereinafter). 
...
For example, \cite{Ozyurt2016} shows that in the euro area, the pass-through is partial and has declined in the 2000s. The lowest degree of pass-through is found in Germany, most likely reflecting the large size of the German economy and the high share of local currency pricing. 
...
Our results also concur with those in \cite{Aydogus2018}, which examine the pass-through to the domestic price level through an input–output model for 26 countries, using I-O tables. 
...
We analyze the contribution of these different products to the HCE deflator's elasticity and document cross-country heterogeneity in the elasticity.\\
%Decomposition et canaux de transmission
...
These two channels explain three-quarters of the transmission of an appreciation in the exchange rate to domestic prices.
By contrast, other channels reflecting the impact of global value chains (such as the price of exported inputs feeding through imported foreign production) play a more limited role, with marked cross-country heterogeneity.
...
We obtain a reliable estimate of the HCE deflator elasticity up to 2019.
...
The Leontief's production model analyzes the impact of a demand shock in a closed economy \citep{Leontief1951} and is broadly used in multi-sector, single-country macroeconomic models. 
...
Leontief's production model has a dual: the cost-push price model, which helps to analyze the consequences of changes in production prices.
...
Traditionnaly, WIOTs are interpreted in a Leontief framework using Leontief production functions to analyze the evolution of quantities in the economy. 
...
We also estimate the inflationary impact on countries that directly and indirectly, through third countries linkages, consume inputs from country $A$. Hence, we will also analyze the impact of an appreciation of the US dollar.
...
\cite{Aydogus2018} examine the pass-through to the domestic price level through an input–output model for 26 countries and 27 sectors, using I-O tables from the OECD Stan Database. 
...
The estimated pass-through of an exchange rate shock to the CPI ranges from 0.07 for the US to 0.34 for Ireland, with a simple average of 0.18.
...
In this section, we analyze the determinants of the HCE deflator elasticity. 
