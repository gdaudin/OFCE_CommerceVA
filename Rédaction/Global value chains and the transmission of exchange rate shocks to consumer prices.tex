...
In line with the existing literature, we find a modest elasticity, ranging from 0.09 to 0.13 depending on the dataset used. 
The elasticity is stable over time but heterogenous across countries, reflecting different degrees of openness to trade. 
Direct effects through imported consumption and intermediates entering domestic production make the bulk of the transmission of an exchange rate appreciation to the HCE deflator.
Global value chains linkages play a more limited role, with marked across-countries heterogeneity. 
As WIOT are not available for the latest years, we estimate the HCE elasticity from 2015 onwards using trade data and GDP statistics. 
We show that a precise assessment of the HCE elasticity to the exchange rate can be estimated without resorting to WIOT.
...
Despite these symplifying assumptions, our approach helps identifying which countries and sectors are under pressure to adjust their prices when subject to an exchange rate variation.
We also assume that the exchange rate shock is proportional across sectors. \\
%Variation élasticite dans le temps et l'espace
In line with the existing literature, we find a rather modest elasticity of the HCE to the exchange rate.
The elasticity ranges from 0.09 to 0.13 depending on the dataset used.
These values are likely an upper bound. Indeed, the elasticity is estimated under the symplifying assummption of full exchange rate pass-through to import prices and under the  producer pricing assumption. Alternative pricing assumptions (such as, for instance, the pricing-to-market assumption) would entail lower elasticity values.  \\
...
After peaking in 2008, the elasticity slightly declined between 2009 and 2015, following the outbreak of the financial crisis. 
By contrast, the elasticity is heterogenous across countries. It ranges from 0.05 to 0.35, reflecting different degrees of openness to trade. 
In the euro area, the elasticity of the HCE deflator to the euro ranges from 0.07 in Italy to 0.18 in Ireland. 
The elasticity is close to 0.1 in Italy, France, Germany, Spain, Portugal and Greece. It is twice higher for small open economies like Luxembourg, Malta, Slovakia and Ireland.  \\
% Impact par secteur
Building on sectoral data, we examine which sectors experience higher spillovers from global price shocks. 
To do so, we focus on the main components of the HCE deflator (manufacturing goods, services, food and energy). 
We analyse the contribution of these different products to the HCE deflator elasticity to an exchange rate shock.
...
We analyse the composition of the HCE elasticity and the role of global value chains in the transmission of an exchange rate appreciation to the HCE deflator. 
...
We find that the first two channels explain the bulk (around three quarters) of the transmission of an exchange rate appreciation to the HCE.
...
Hence, only one-fourth of the HCE elasticity to the exchange rate is attributable to global value chains participation.

%Extrapolation
The construction of World Input-Output tables is data-demanding and WIOTs are typically released with a lag of several years.
As a result, WIOT are not available for the latest years. The most recent dataset dates back to 2015. 
To address this gap, we estimate the HCE elasticity from 2015 onwards using trade data on consumption and intermediates and GDP statistics.
