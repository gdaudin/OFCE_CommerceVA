...
This paper contributes to the debate on inflation dynamics by measuring the impact of exchange rate shocks on consumer price indexes from 1995 to 2018 by focusing on cost-push inflation through global value chains.
It compare conclusions from three sectoral world input-output datasets (two versions of TiVA and WIOD) in a Cobb-Douglas production and consumption functions in a partial equilibrium setting.
It provides a tool to produce up-to-date estimates of the effects using easy to obtain GDP and trade data. 
Depending on countries, the absolute value of the elasticity of the CPI to exchange rate shocks varies from 5\% to 35\%.
This confirms the importance of global value chains in channeling external shocks into inflation dynamics.
Most of the propagation is throught imported final goods and domestic input-output links.
In a sample of 43 countries, the mean output-weighted elasticity of the CPI to exchange rate shocks according to WIOD increased in absolute value from 7.5\% in 2000 to 9.4\% in 2008.
...
Assuming Cobb-Douglas production and consumption functions where firms have a very simple cost-minimizing behaviour, it is possible to compute a partial-equilibrium effects of an exchange rate shock on consumer price indexes.
...
To convert this vector into a price-level shock in country $i$, $\bar{s}^i$, we assume u single representative consuming household with a Cobb-Douglas consumption function.
Assuming usual household maximising behaviour that lead to equalize marginal utility and prices, the price of his consumption basket will increase by the weighted average of the sectoral effects of the shock.
Let $HC^i$ be the vector of sectoral shares in country $i$'s consumption\footnote{Consumption is at market prices, whereas the WIOTs we have worked with until now are all at basic prices. This is not an issue as long as we assume that all taxes and subsidies on products are proportional, as is the main one, the value-added tax.} :
\begin{eqnarray*}
HC^i=\left( 
	\begin{array}{c}
	\frac{{hc}_{11}^i}{hc^i} \\
