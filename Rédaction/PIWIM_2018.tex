...
For example, in the absence of global value chains, the only way currency appreciation directly affects consumer prices is through a reduction in the price of imported consumption goods and hence consumer prices.
But if there are global value chain and hence imports of intermediate goods, currency appreciation has an effect on domestic production costs that further reduces consumer prices.
The other way around, because of exported intermediate goods, inputs become more expensive for foreign firms.
That increases their production costs and, presumably, also increases domestic consumer prices.
...
After showing how to determine the accounting effect of exchange rate fluctuations using three different sources for World Input-Output tables (WIOT hereafter), we  demonstrate that the findings of this data-hungry exercise can be extrapolated through time to relatively data-poor recent periods.
This is important for economic forecasters in various institutions who want to evaluate the role of supply chains diffusion of price shocks in current economic events.
