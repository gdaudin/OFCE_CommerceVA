...
Figure \ref{fig:share_impt} shows, for example, that the contribution of imported consumption goods is higher than 0.5 in most countries. While imported consumption has a smaller share in total consumption, this is over-compensated by the sensitivity of its prices to exchange rate shocks, as show by the comparison of \ref{fig:intensity_dom} and \ref{fig:intensity_impt}.

Similar decompositions of the matrix $HC^i$ can be done to analyse the contribution of different sectors. That allows the identification of the contribution of different sectors to the inflation shocks in different countries.
...
One might wonder whether using world input-ouput matrices provided by WIOD or TIVA to compute the effect of exchange rate shocks on household consumption prices is is worth the wait (the latest version of WIOD covers 2014) and the additional complexity. To answer this question, we  decompose  $\overline{s_{i}}^{i,HC}$ into different elements classified by ease of use and computation. Let us start from equation \ref{eq:eqdomcurrency}.

\begin{equation}
\begin{array}{lccl}
	S^ i&=&C^i	&+ \left(\hat{C}^i_\$.{\cal B}+{C^i}{\tilde{{\cal B}}}\right)*{{(I-{\cal A})}^{-1}} \\
	S^i &=&\underbrace{C^i}_{\substack{\text{(E1) initial effect on} \\ \text{ imported consumption}}}&+ \underbrace{{C^i}{\tilde{{\cal B}}}}_{\substack{\text{(E2) direct effect on} \\ \text{ domestic consumption} \\ \text{ produced with  imported inputs}}}  + \underbrace{\hat{C}^i_\$.{\cal B}}_{\substack{\text{(E3) indirect effect on} \\ \text{imported consuption} \\ \text{produced with domestic inputs}}} \\ &&+\underbrace {\left( \hat{C}^i_\$.{\cal B} + {C^i}{\tilde{{\cal B}}}\right)*{{(I-{\cal A})}^{-1}}*{\cal A}^{-1}}_{\text{(E4) residual}} \\
\end{array}
\end{equation}

When the shock is an appreciation of the domestic currency, $E1$ and $E2$ reduce country $i$'s household consumption prices and $E3$ increases country $i$'s household consumption prices. Notice that $E1$ and $E2$ are easy to compute with national input-output matrices, whereas computing $E3$ and $E4$ require the use that world input-output matrices.
