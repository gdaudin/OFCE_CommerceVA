...
The elasticity is stable over time but heterogeneous across countries, ranging from 0.05 to 0.22. 
Such heterogeneity reflects different degrees of openness to trade and differences in foreign product content in domestic consumption. 
...
As WIOT are not available for the latest years, we compute the HCE deflator elasticity from 2016 onwards using trade data and GDP statistics. 
By doing so, we compute a precise estimate of the elasticity without resorting to WIOTs.
...
A country’s import, producer or consumer prices change in response to a change in its exchange rate. 
...
%Data
In this paper, we analyse the impact of exchange rate variations on domestic prices using several datasets covering most advanced and emerging economies, from 1995 to 2018. 
%Method
...
This modest estimate is likely an upper bound. Indeed, we make a number of assumptions to simplify our computations.
First, we assume that exchange rate fluctuations completely pass-through to import prices.
...
We find that the impact of exchange rate variations on consumer prices has remained broadly stable over the past two decades. 
...
The highest impacts are obseverd for the US's major trading partners (Canada, Mexico and Ireland).\\
% Impact par secteur
...
The last two channels, which reflect the impact of participation in global value chains, play a more limited role, with marked across-countries heterogeneity.\\
%Extrapolation
...
%We assume a Cobb-Douglas production framework where firms have a simple cost-minimising behaviour and work in a partial equilibrium setting.
We make three assumptions to simplify our computations. 
...
Second, we suppose that all pricing occur using the currency of the producing country, despite the well-documented role of dominant-currency pricing (\cite{Gopinath2020}).
...
First, we analyse the evolution of the elasticity of the HCE deflator to the exchange rate. We document differences accross countries.
...
We analyse the contribution of these different products to the HCE deflator elasticity and analyse cross-country heterogeneity in the elasticity.\\
%Decomposition et canaux de transmission
...
%Hence, a currency appreciation reduces the price of imported inputs, and thus further reduces domestic consumer prices. 
\textit{iii)} the price of exported inputs feeding through imported foreign production;
\textit{iv)} changes in domestic and foreign production costs in turn pass through to the price of inputs for domestic and foreign goods, causing further changes in production costs through input-output linkages.
We find that the first two channels explain three-quarters of the transmission of an exchange rate appreciation to domestic prices.
...
%Extrapolation
Fourth, we show that a precise assessment of the HCE deflator elasticity to the exchange rate can be estimated without resorting to WIOTs. 
The construction of World Input-Output tables is data-demanding and WIOTs are typically released with a lag of several years.
As a result, WIOT are not available for the most recent years. For instance, the latest WIOD dataset dates back to 2015. 
...
In section \ref{sec:prixconso}, we estimate the elasticity of the HCE deflator to the exchange rate up to 2018 and analyse its determinants.
...
\paragraph{The Input-Output model applied to a change n production costs} 
\label{subsec:io}
...
Firms' margins are assumed to be fixed. Prices only adjust to absorb cost changes and production techniques are fixed during successive production cycles.
Inputs substitution (for instance, between countries producing the same goods) is not accounted for, despite a change in relative prices.
Although the division of global value chains largely takes place within multinational firms, supply-driven price models assume a unique pricing system based on market prices and independent of firms' strategies. 
...
Traditionnaly, WIOTs are interpreted in a Leontief framework of Leontief production functions to analyse the evolution of quantities in the economy. 
...
We derive a price equation, following \citep{DeSoyres2018}).\\
...
The higher the country-specific industries'reliance on those inputs, the higher the change in prices.\\
...
If the change in the exchange rate is small enough, the elements of $S$ correspond to the elasticity of sectoral prices to a change in input prices. 
Using consumption shares to perform a weighted average of these elasticities, we compute the household consumption expenditure (HCE) deflator elasticity to a change in input prices \footnote{The HCE deflator has different sectoral weights than the headline consumer price index (CPI) as the former also encompasses, for example, the rental equivalent of real estate expenditures.}.
...
It contains annual information for 56 industries. Therefore, for each year, a full country-sector input-output matrix traces the importance of a supplying industry in country $j$ for an industry in country $i$. The values in WIOTs are expressed in U.S. dollars at basic prices. Market exchange rates were used for currency conversion \citep{Timmer2015}. 
...
\centering
\centering
\caption{\small{\textbf{Coverage of the WIOD}}}
\small
\begin{tabular}{ll}
\hline\hline
...
\end{threeparttable}
\end{table} 
\paragraph{The Asian Development Bank's MRIO database} is an extension of WIOD providing a larger coverage of Asian economies.
The WIOD provides disaggregate information for a limited number of Asian economies. 
To address the analytical needs related to the Asian and Pacific regions, the Asian
Development Bank (ADB) has augmented the WIOD with details for nineteen additional Asian economies\footnote{The MRIO database includes the following additional economies: Bangladesh, Bhutan, Brunei Darussalam, Cambodia, Fiji, Hong Kong, Kazakhstan, Kyrgyz Republic, Lao People’s Democratic Republic, Malaysia, Maldives, Mongolia, Nepal, Pakistan, Philippines, Singapore, Sri Lanka, Thailand and Viet Nam.}. 
The industrial aggregations and the conceptual framework of the ADB MRIO database (and hence the methods used in the construction of the
tables) are similar to those of WIOD.
...
The 2016 edition of the TiVA database (third revision) includes 64 economies covering OECD, EU28, G20, most East and South-east Asian economies, a selection of South American countries and the Rest of the world. The industry list includes 34 sectors, among which 16 manufacturing and 14 services sectors. 
...
\centering
\centering
\caption{\small{\textbf{Coverage of TiVA revisions 3 (2016) and 4 (2018)}}}
\small
\begin{tabular}{ll}
\hline\hline
Europe & Austria, Belgium, Bulgaria, Croatia, Cyprus, Czech Republic, Denmark,\\
...
WIOD and the TiVA database are constructed using similar assumptions (\cite{OECD2011,Timmer2015,OECD2018}).
...
The TiVA databases do not rely on this assumption for two countries (Mexico and China). Instead, the TiVA databases use different input-output tables for the exporting, import processing and domestic-market oriented sectors of these countries (or at least two of those, depending on the country and the version of the TiVA database). Despite this difference, we obtain similar results with TiVA and WIOD, which probably reflects the fact that we do not focus on Mexico and China.\\
...
Implementing an exchange rate variation is more complex than implementing a change in production costs. 
...
We are primarily interested in the impact of an exchange rate appreciation on consumer prices in country $A$. 
...
It has an impact on consumer prices (in national currency) in country $A$.\\
Note that, for the sake of simplicity, we make the implausible hypothesis that producers completely pass the exchange rate appreciation on their production prices.  
...
Similarly, settlement and invoicing of imports in the domestic currency is another factor likely to weaken the elasticity of domestic prices to exchange rate movements.
For instance, assuming that only 60\% of invoices in the euro area goods trade are denominated in foreign currency (see \cite{Ortega2020}) would entail lower elasticity values.
To anticipate on our results (see Figure \ref{fig:decompositionofs}), reducing the exchange rate pass-through on consumption goods will entail lower elasticity values than reducing it on inputs.
...
The overall impact of the exchange rate variation in dollar terms is equal, for the shocked country $A$, to the rise in prices due to the exchange rate fluctuation, minus direct and indirect decreases (via inter-industry linkages in the country), in national currency and then converted back into dollar terms, in the prices of inputs imported from $B$ and disseminated to all branches. 
...
We approximate these changes by their log point equivalent\footnote{We could have used the log point approximation earlier and write that $c^i=-c_\$^i$ with nearly the same results for small shocks.}.
...
Compared to $\cal A$, we "close off" the links between a domestic input price shock and the price of goods as well as the link between non-shocked input prices and the price of goods in a non-shocked country.\\
...
The first-step impact corresponds to the product of the shock vector $C^i$ and a matrix ${\tilde{{\cal B}^i}}$. ${\tilde{{\cal B}^i}}$ builds on the large matrix ${\cal A}$ of which only the country blocks of those inputs imported by country $i$ from other countries have been retained. 
...
We then convert the direct impact into dollars, by multiplying it by the new value of the national currency in dollars, $\left(1+c_\$^i\right)$. The direct impact of the appreciation of the shocked country $i$ on the price of its inputs corresponds, in dollars, to ${\tilde{C}_\$^i}{\tilde{{\cal B}^i}}$ with: 
\begin{eqnarray}
...
The change in input prices then spreads to all sectors in all countries via the global intersectoral exchanges transcribed by the matrix of technical coefficients of the large matrix $\cal {A}$. This process will be simultaneously repeated several times, until the effects are completely exhausted.
In the end, the total price effect of the exchange rate variation is equal to the sectoral shock itself, incremented by changes in input prices due to changes in imported input prices (both in the shocked country and in non-shocked countries), and by all changes in prices during the production processes, i. e.:\\
\begin{eqnarray*}
...
\label{eq:eqinternationalcurrency}
 \end{eqnarray}
With $S_\$^i$ the total impact vector composed of the elements ${{{s}}^i_{\$kj}}$ showing the total impact of a change in the exchange rate of country $i$ on the price of the country $k$'s sector $j$ in international currency in log points. 
...
Let $HC^i$ be the vector of sectoral shares in country $i$'s household consumption\footnote{Consumption is at market prices, whereas the WIOTs are all at basic prices. This is not an issue as long as we assume that all taxes and subsidies on products are proportional, like the value-added tax.}:
\begin{eqnarray*}
HC^i=\left( 
	\begin{array}{c}
	\frac{{hc}_{11}^i}{hc^i} \\
...
$\overline{s_{i}}^{i,HC}$ provides the elasticity of the HCE deflator of country $i$ to the exchange rate. 
 \begin{eqnarray}
...
Figure \ref{fig:comp_WIOD_TIVA} shows that we obtain similar country results with the WIOD and two distinct releases of TiVA, both in 2011 and 2014.
\begin{figure}[H]
\centering
...
The model can also track the effect of movements in the currency of other countries.
For illustration, we estimate the impact of an appreciation of the US dollar (USD) on its trading partners using WIOD.
This exercise assumes that exports are always invoiced in the exporter’s currency.
It does not take into account the large role of the dollar in international trade invoicing.\\
The US example illustrates that countries are affected in proportion to their trading links with the country which currency appreciates.
We obtain the highest elasticities for the neighbouring countries of the US. 
The elasticity of the HCE deflator to the USD is close to 0.12 for Canada and 0.09 for Mexico (see Figure \ref{fig:WIOD_HC_elasticities_USD}).
...
The U.S. is Ireland's major trading partner outside of the EU. According to the Irish Central Statistics Office, the U.S. accounted for 19\% of the total current account outflows in 2018, the largest share of any other EU country\footnote{However, a large portion of US imports (pharmaceuticals products and aircraft) are later exported by Irish-based firms without being purchased by Irish consumers (\cite{Reddan2017}), and therefore have a negligible contribution to domestic consumer goods inflation.}. 
...
While the latest dataset available for WIOD dates back to 2014, MRIO covers the most recent years, up to 2018. 
Results from the MRIO database suggest that the elasticity has bounced back from 2016 onwards, reaching 0.09 in 2018.
However, MRIO suffers from data quality issues in 2018 (see Online Appendix F). Between 2017 and 2018, the HCE deflator elasticity sharply increased in a number of countries (e.g. China\footnote{For instance, we observe large shifts in the consumption of services in China (see Online Appendix F), suggesting data quality issues in 2018.} and India). 
Hence, the elasticity estimated with MRIO is not reliable for this last year.\\
...
Figure \ref{fig:PIWIM_LONGITUDINAL} shows that output-weighted elasticities are lower than arithmetic means, reflecting the fact that large countries are relatively closed compared to small economies.

\begin{figure}[H]
	\centering
	\caption{\footnotesize{\textbf{Evolution of the HCE deflator elasticity, 1995-2018}}}
	\begin{tabular}{c}
...
\subsection{Determinants of the HCE deflator elasticity}\label{subsec:channels}
...
%Hence, a currency appreciation reduces the price of imported inputs, and thus further reduces domestic consumer prices. 
\textit{iii)} the price of exported inputs feeding through imported foreign production;
\textit{iv)}, changes in domestic and foreign production costs in turn pass through to the price of inputs for domestic and foreign goods and cause further changes in production costs through input-output linkages.\\

Figure \ref{fig:decomp_origine} shows the contribution of domestic versus imported goods to the elasticity of the HCE deflator.
...
Similarly, domestic core inflation (i.e. all products except food and energy) accounts for a significant share of the total impact (Figure \ref{fig:decomp_sectxorigin}), reflecting the weight of domestic services and non-energy industrial goods in total consumption.
...
\begin{figure}[H]
	\centering
	\caption{\footnotesize{\textbf{Contribution of different products to the HCE deflator elasticity}}}
	\begin{tabular}{c}
		\includegraphics[width=4.5in, height=3in]
		{decomp_sect.png}\\.
	\end{tabular}
...
We estimate the HCE deflator elasticity from 2016 onwards using GDP statistics and trade data on consumption and intermediates.\\
...
$C^i$ and $\hat{C}^i_\$$ have a large number of zeros. Hence, we can write, defining $HC^{i,dom}$ and $HC^{i,imp}$ as the domestic and imported shares of $HC^i$ and adjusting the dimension of $E1$, $E2$ and $E3$:
...
When the domestic currency appreciates, $E1.HC^{i,imp}$ ($E1.HC$ hereafter) and $E2.HC^{i,dom}$ ($E2.HC$ hereafter) reduce the consumer prices of the country $i$ whereas $E3.HC^{i,imp}$ ($E3.HC$) increases them. 
...
$E1.HC$ dominates and $E2.HC$ is consistently important.
While $E3.HC$ is negligible, $E4.HC$ accounts for 10\% to 30\% of $\overline{s}_{i}^{i,HC}$ for most countries. The less open to trade is a country, the larger the share of $E4.HC$. \\
On the whole, as shown by Figure \ref{fig:shareofs}, input-output mechanisms (i.e. all shares except $E1.HC$) explain a large share of the elasticity, especially for large countries or euro area countries. This share has increased until 2013-2014, implying an increasing need of data from WIOTs to perform our computations (see Figure \ref{fig:shareofsthroughtime}).
...
Still, $E1.HC$ and $E2.HC$ are relatively important (especially for small countries). This means, for example, that although the model as it stands cannot easily accommodate simultaneous exchange rate changes in different currencies (as that would require increasing the number of $\cal{B}$ matrices and $C$ vectors) we can expect their effect to be correctly approximated through a own-currency exchange rate change of the trade-weighted average of the country-specific exchange rate changes.\\
It must also be noted that $E1.HC$ and $E2.HC$ can be computed with national input-output matrices, whereas world input-output matrices are needed for computing $E3.HC$ and $E4.HC$. 
...
We infer $\overline{s}_{i}^{i,HC}$ from $E1.HC$ and $E2.HC$.
...
This likely reflects the fact that a small economy compensates for its small size by being more open, and vice versa.\footnote{Although this functional form might seem counterintuitive (one might expect that the elasticity is an affine function of openness as summarized by $E1.HC$), the analytical examination of the two-country, one-sector case shows that it is plausible (see Online Appendix E).}
...
The sum of the share of imported goods in household consumption and the share of imported inputs in household consumption of domestic goods ($E1.HC + E2.HC$) are a good predictor of the impact of exchange rate fluctuations on household consumption prices. 
...
We rely on a proxy (see Online Appendix C) by identifying consumption and intermediary goods imports using the CEPII BACI database\footnote{BACI provides disaggregated data on bilateral trade flows for more than 5000 products and 200 countries. The database is built from data directly reported by each country to the United Nations Statistical Division (Comtrade). Products are defined as items from the Harmonized System nomenclature, at the 6-digit level.} and the BEC classification and using a measure for household consumption (from the World Bank) and intermediate consumption (from Eurostat).
As intermediate consumption data is not available for all countries, the regressions provided in Online Appendix C only include a limited number of observations.

To expand our panel, we use in this section an even simpler proxy for $E1.HC+E2.HC$. 
This proxy only requires trade data from BACI and GDP data from the World Bank. This data is available until 2018. 
...
Using these equations, we can predict the HCE deflator elasticity from 2016 onwards.
...
We also compare our predictions with the elaticity estimated using MRIO, which provides WIOTs up to 2018. 
Our predictions are in line with results from MRIO up to 2016. For the most recent years, MRIO is fraught with data quality issues (see Online Appendix F), which makes the comparison irrelevant. 
...
Our contribution to the literature is fourfold. \\
First, we analyse the composition and determinants of the HCE deflator elasticity using world input-output tables covering twenty years of data, from 1995 to 2018. 
...
Direct effects through imported consumption and intermediates entering domestic production explain three-quarters of the transmission of an exchange rate appreciation to domestic prices.
...
To address this data gap, we estimate the HCE deflator elasticity using GDP statistics and trade data on consumption and intermediates.
