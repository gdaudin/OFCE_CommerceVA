...
\begin{document}
\title{Global value chains and the transmission of exchange rate shocks to consumer prices	\thanks{We thank Pavel Diev, Hubert Escaith, Guillaume Gaulier, Yannick Kalantzis, Guy Levy-Rueff and Jean-François Ouvrard for comments and suggestions, as well as participants at Banque de France seminars.}\\
\vspace{1cm}
...
Global value chains strengthens cross-country transmission of price shocks through cost-push inflation.
...
We assume a Leontieff framework with no substitution between different consumer goods or inputs.
Firms react to price changes of inputs by adjusting their prices to maintain their value-added. 
In a sample of 43 countries, the mean output-weighted elasticity of the CPI to exchange rate shocks according to WIOD increased in absolute value from 0.075 in 2000 to 0.094 in 2008.
It then declined to 0.088 in 2014.
Both recent versions of TIVA yield a similar evolution.
An issue with our approach is that world input-output databases are only available after a sizeable time gap (the latest data is from 2015). 
However we show that it is possible to use up-to-date GDP and trade data to extrapolate the impact of exchange rate shock on consumer price index.
...
Despite this shortcomings, the accounting approach provides a first approximation measure of the contrasted suceptibility of different sectors and different countries to exchange rate shocks. 
Hence, though unrealistic, it is useful for identifying which countries and sectors are under pressure to adjust their prices when subject to exchange rate shocks.\\
\\
In this paper, we measure the accounting effect of exchange rate fluctuations on consumer price indexes. We use world input-output tables covering twenty years of data (from 1995 to 2015).\\
...
It then declined to 0.088 in 2014.
Both recent versions of TIVA yield a similar evolution.
Input-output mechasims (i.e. not the change in the prices of imported final goods sold directly to domestic consumers) explain a large share of the elasticity, especially for large countries or countries of the Eurozone subject to a shock on the Euro.\\
%GD 20200619 c’est pas net ce 1st/2nd round.  Je laisse tomber pour quelque chose de plus central pour le papier
...
As a result, at the time of wrinting, the latest WIOT is five year old (2015).
...
Using the WIOD database, we find that a 100\% appreciation of the dollar reduces the U.S. CPI by 5.5\%. 
...
On the whole, as shown by Figure \ref{fig:shareofs}, Input-output mechasims (i.e. everything but E1HC) explain a large share of the elasticity, especially for large countries or countries of the Eurozone subject to a shock on the Euro.
...
Our results suggest that we can approximate the elasticity of consumer prices for the most recent years, using the share of imported goods in household consumption and the share of imported inputs in household consumption of domestic goods. E1HC+E2HC are a good predictor of the total effects.
Yet, care should be taken in extrapolating them as most of the other effects (E3HC+E4HC) add to the first round rather than amplifying them.
They are of similar size for small open economies and large closed ones.
It might be that the small economy counterbalances its large openness rate and vis-versa.\footnote{The functional form might seem a bit counterintuitive (one might expect that the elasticity is affine function of openness as summarized by E1), but the analytical examination of the two-country, one sector case, shows that it is plausible (see Appendix \ref{AnalyticalAppendix}).}
...
While the World Bank provides regular estimates for household consumption, it does not provide an estimate for intermediate consumptions. Eurostat provides estimates for intermediate consumptions in the case of European countries\footnote{And some others for selected years -- préciser la source}. 
...
Yet, estimating successive cross-sections is a demanding test to establish a link between the elastiticy computed by PIWIM based on WIOD data and "public" data assembled from various sources. It does not allow to exploit country-specific information on the determinant of the elasticity. A less demanding test is to run a panel with country fixed-effects, assuming that $\beta$ is constant over time but that it explains only within-country variations (see equation \ref{eq:eq9}). 
...
\begin{figure}[!h]
	\centering
	\caption{\footnotesize{\textbf{Comparing the CPI elasticity in 2014 (WIOD) and the prediction from a panel regression on the 2000-2008 period with fixed effects using Eurostat data. Beta is constant over time for each country}}}
	\begin{tabular}{c}
...
\begin{figure}[!h]
	\centering
	\caption{\footnotesize{\textbf{Comparing the CPI elasticity in 2014 (WIOD) and the prediction from a panel regression on the 2000-2008 period with fixed effects without using Eurostat data. Beta is constant over time for each country}}}
	\begin{tabular}{c}
...
%Yet, care should be taken in extrapolating them as most of the non-first round effects add to the first round rather than amplifying them.
%They are of similar size for small open economies and large closed ones.
%In a sample of 43 countries, the mean output-weighted elasticity of the CPI to exchange rate shocks according to WIOD increased in absolute value from 0.075 in 2000 to 0.094 in 2008.
%It then declined to 0.088 in 2014.
%Both recent versions of TIVA yield a similar evolution.
