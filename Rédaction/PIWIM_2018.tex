...
World input-output databases are not available for the last five years. 
To address this gap, we use up-to-date GDP and trade data to approximate the accounting impact of an exchange rate shock on consumer prices for the most recent years.
...
When value chains are global, fluctuations in exchange rates aﬀect domestic prices through two distinct price channels: \textit{i)} the prices of imported ﬁnal goods sold directly to domestic consumers and \textit{ii)} the prices of imported inputs feeding through domestic production of intermediate and ﬁnal goods.
...
World input-output databases are not available for the last five years. 
To address this gap, Section \ref{sec:extrapo} relies on up-to-date GDP and trade data to approximate the accounting impact of an exchange rate shock on consumer prices from 2015 onwards.
...
\cite{Georgiadis2019} estimate that the rise of global value chain (GVC) participation accounts for 50\% of the decline in the ERPT to import prices observed since the mid-1990s. 
...
\cite{Cochard2016} relies on an accounting approach to analise the effect of costs on prices ("cost-push inflation"). 
Firms' margins are assumed to be fixed. Prices only adjust to absorb cost changes, production techniques are fixed during successive production cycles and inputs substitution (for instance, between countries producing the same goods) is not accounted for, despite variations in relative prices.
...
% vous citez aussi bien tiva et wiod dans les résultats, je suggère donc de rebaptiser le titre
\subsection{Data}
\label{subsec:data}
...
WIOTs show which foreign industry produced a specific good, and which foreign industry or final user uses the exports of a given country. 
...
Two datasets are available: (\textit{i}) the World Input Output Database (WIOD) and (\textit{ii}) the OECD-ICIO database TiVA.

\paragraph{The World Input Output Database (WIOD)}
WIOD contains time series of inter-country input-output tables from 2000 to 2014. 
...
The TiVA database is compiled by the OECD and the WTO. It builds on the OECD harmonized individual country input-output tables to provide matrices of inter-industrial flows of goods and services in current prices (USD).
...
We also estimate the inflationary impact on countries that directly and indirectly consume, through third countries linkages, inputs from country $A$.\\
...
Assuming a 100$\%$ appreciation of the currency of country $A$ against the other two currencies, the production prices of country $A$ expressed in dollars double compared to those of country $B$ expressed in dollars. 
...
Conversely, the prices of imported inputs in country $A$ remain constant in dollars, since the production prices of country $B$ have not changed, and fall by half once expressed in country $A$'s national currency.\\
...
A 100$\%$ appreciation in the exchange rate of a currency against other currencies therefore corresponds to an absolute shock of $+1$, with production prices in the shock-stricken country rising from 1 to 2 dollars.\\
...
In matrix notation, we create two matrices that build on the world input-output matrix $A$ defined in \ref{subsec:ioprice}. 
These two matrices retain only the direct effects of the exchange rate shock on the price of goods imported by the shocked country $i$ and the direct effects of the exchange rate shock on the price of goods imported by the rest of the world from country $i$. 
To formalise the initial impact of the shock on the price of traded goods, we neutralize the impact of an input price shock on the price of domestic inputs as well as on the price of inputs traded between countries that are not shocked.\\
...
\label{eq:eqinternationalcurrency}
 \end{eqnarray}
With $S_\$^i$ the total impact vector composed of the elements ${{{s}}^i_{\$kj}}$ showing the total impact of a shock on country $i$'s exchange rate on country $k$'s sector $j$. 
...
To illustrate the difference, the two-country, one sector case is explored in Appendix 1.
...
We use the model presented in \ref{subsec:chocchange} to analyse the impact of an exchange rate shock on consumer prices.
We focus on the absolute value of $\overline{s_{i}}^{i,HC}$ following a 100\% appreciation of the domestic currency versus all other currencies. 
Following an apreciation of the domestic currency, imported inputs and imported consumer goods become cheaper.
Hence, domestic prices expressed in domestic currency decrease. 
...
Using the WIOD database, we find that a 100\% appreciation of the dollar reduce the U.S. consumer prices by 5.5\%. 
...
For France, using WIOD, a 100\% appreciation of the euro leads to a 7.6\% reduction of domestic prices. 
A 100\% appreciation of the hypothetical French national currency leads to a 12.2\% reduction of domestic prices. 

Using a sample of 43 countries, we plot the elasticity of consumer prices over time.
Figure \ref{fig:comp_WIOD_TIVA} shows that the evolution is similar, regardless of the database (WIOD or TIVA).
...
However, using data from TIVA rev3 yields a slightly higher elasticity (see Figure \ref{fig:PIWIM_LONGITUDINAL}).

\begin{figure}[!h]
	\centering
	\caption{\footnotesize{\textbf{Comparing the elasticity of consumer prices to an exchange rate appreciation of 100\% for WIOD and TIVA, 1995-2014}}}
	\begin{tabular}{c}
...
\begin{figure}[!h]
	\centering
	\caption{\footnotesize{\textbf{Distribution of the elasticity of consumer prices to a 100\% exchange rate appreciation (WIOD)}}}
	\begin{tabular}{c}
...
We look at the contribution of domestic versus imported goods to the total impact of an exchange rate shock on consumer prices.
...
Figure \ref{fig:decomp_origine} shows that the prices of imported final consumer goods contribute most to the total effect.
...
We also analyse the impact of global inflationary shocks on the main components of consumer prices (manufacturing goods, services, food and energy).
...
Similarly, domestic core inflation accounts for a significant share of the total impact (Figure \ref{fig:decomp_sectxorigin}), reflecting the weight of domestic services and non-energy industrial goods in total consumption.
...
\section{Can we extrapolate the consumer price elasticity?}
\label{sec:extrapo}
...
Given these difficulties, we look for a simpler way to compute the elasticity of consumer prices to the exchange rate.
...
When the domestic currency appreciates, $E1$ and $E2$ reduce country $i$ consumer prices whereas $E3$ increases them. 
...
Equation \ref{equ:decomp_impexp} focuses on the contribution of domestic versus imported goods to the total impact of an exchange rate shock on consumer prices. 
...
World input-output matrices are not available for the most recent years. 
Yet $E4.HC^i$ can be inferred from easier-to-compute elements of $\overline{s}_{i}^{i,HC}$.
...
\begin{figure}[!h]
\centering
\caption{\footnotesize{\textbf{Evolution of $\beta$ and $R2$ over time (WIOD)}}}
\begin{tabular}{c}
...
\begin{figure}[!h]
\centering
\caption{\footnotesize{\textbf{Evolution of $\alpha$ over time (WIOD)}}}
\begin{tabular}{c}
...
We have to use a proxy. It is fairly easy to identify consumption and intermediary goods imports using UN Comtrade data and the BEC classification. The World Bank provides regular estimates of household consumption.%\footnote{préciser la source??}. 
It does not provide an estimate for intermediate consumptions. In the case of European countries\footnote{And some others for selected years}, Eurostat provides estimates for intermediate consumptions.%\footnote{préciser la source}.
...
% le passage ci-dessous est un vrai défi! il faut vraiment reformuler, vérifiez svp si ma traduction est ok
%Yet, estimating successive cross-sections is a demanding test to establish a link between the elastiticy computed by PIWIM based on WIOD data and "public" data assembled from various sources. It does not allow to exploit country-specific information of the link between the elasticity and public data. A less demanding test is to run a panel with country fixed-effects, assuming that $\beta$ is constant over time but that it applies only to within-country variations of the proxy for E1HC + E2HC (see equation \ref{eq:eq9}). 
...
We estimate the elasticity of domestic consumer prices to a shock on the domestic currency.
...
% plutôt que de dire que c'et intriguant il faudrait suggérer une explication Intriguingly, most of the non-first round effects add to the first round effects rather than amplifying them.
% un peu abscons  We show numerically that something similar exists in the two-country, one sector case. 
...
To measure the effect on French consumption prices, we compute a weighted sum of these effects.
...
When the shock corresponds to an appreciation of the domestic currency, $E1$ and $E2$ reduce country $i$'s consumer prices whereas $E3$ increases it. $E1$ and $E2$ are easy to compute with national input-output matrices, whereas world input-output matrices are needed for computing $E3$ and $E4$.
...
Let us see what happens in a two-country, one-sector economy : 
...
Figure \ref{fig:tous_les_E} comfirms that, in that numerical exercice, the total effect is dominated by the direct effect through imported consumption goods and, to a lesser extent, the effect on domestic consumption goods through imported inputs. The other effects are approximately additive if the openness rate is betweem 0.15 and 0.5.
