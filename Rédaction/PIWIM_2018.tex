...
% je propose de présenter le débat, sinon il est difficile de comprendre l'abstract en l'état, je trouve néanmoins le débat assez éloigné du papier, qui ne se focalise pas sur les pays avancés: les wiots icluent des pays émergents peu concernés par le débat sur la courbe de phillips
...
We build on three sectoral world input-output datasets (two versions of TiVA and WIOD) using Cobb-Douglas production functions in a partial equilibrium setting.
It provides a tool to produce up-to-date estimates of the effects using easy to obtain GDP and trade data. 
Depending on countries, the absolute value of the elasticity of the CPI to exchange rate shocks ranges from 5\% to 35\%, confirming the importance of global value chains in channeling external shocks into inflation dynamics.
Most of the propagation of shocks is throught imported final goods and domestic input-output linkages.
...
Two distinct puzzles have emerged. 
The "missing disinflation episode” emerged during the economic dowturn (2009-2012), when inflation remained much higher than suggested by the high level of economic slack. 
...
\noindent\fbox{%
	\parbox{\textwidth}{%
		...
		\item ${\cal A}$: Matrix of world input-output coefficients.
				\item ${\cal B}^i$: ${\cal A}$ including only the coefficients of each country’s sectoral inputs imported from country $i$ (not including domestic inputs in $i$).
				\item ${\tilde{{\cal B}^i}}$: ${\cal A}$ including only the country blocks of the inputs imported by country $i$ from other countries have been retained (not including domestic inputs in $i$).
